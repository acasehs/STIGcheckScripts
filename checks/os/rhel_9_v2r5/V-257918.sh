...
#
# Check Content:
#     Verify the system commands contained in the following directories are owned by "root" with the following command:  $ sudo find -L /bin /sbin /usr/bin /usr/sbin /usr/libexec /usr/local/bin /usr/local/sbin ! -user root -exec stat -L -c "%U %n" {} \;  If any system commands are found to not be owned by...
#
# Exit Codes:
...
echo "TODO: Implement check for V-257918"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 9 system commands must be owned by root.",
  "status": "$STATUS",
