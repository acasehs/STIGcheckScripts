...
Check Content:
    Verify RHEL 9 is configured to audit the execution of the "execve" system call with the following command:  $ sudo auditctl -l | grep execve  -a always,exit -S arch=b32 -S execve -C uid!=euid -F euid=0 -F key=execpriv -a always,exit -S arch=b64 -S execve -C uid!=euid -F euid=0 -F key=execpriv -a alw...
...
SEVERITY = "medium"
RULE_TITLE = """RHEL 9 must audit uses of the "execve" system call."""
