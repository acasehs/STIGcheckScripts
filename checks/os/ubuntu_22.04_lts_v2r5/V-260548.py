...
Description:
Temporary accounts are privileged or nonprivileged accounts established during pressing circumstances, such as new software or hardware configuration or an incident response, where the need for prompt
...
def perform_check(config):
    ...
    # Check Content: Verify temporary accounts have been provisioned with an expiration date of 72 hours by using the following command: 
 
     $ sudo chage -l <temporary_account_name> | grep -E '(Password|Account) expir
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must automatically expire temporary accounts within 72 hours.')
