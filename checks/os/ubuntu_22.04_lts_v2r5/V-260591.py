...
def perform_check(config):
    ...
    # Check Content: Verify the "auditd.service" is enabled and active by using the following commands:  
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must produce audit records and reports containing information to establish when, where, what type, the source, and the outcome for all DOD-defined auditable events and actions in near real time.')
