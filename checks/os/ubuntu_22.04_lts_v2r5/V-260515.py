...
def perform_check(config):
    ...
    # Check Content: Verify the ufw is enabled on the system with the following command:  
  
     $ sudo ufw status 
     Status: active 
  
If the above command returns the status as "inactive" or any type of error, thi
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must enable and run the Uncomplicated Firewall (ufw).')
