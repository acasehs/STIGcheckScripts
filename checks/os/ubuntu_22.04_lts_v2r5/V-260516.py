...
def perform_check(config):
    ...
    # Check Content: Verify the Uncomplicated Firewall (ufw) is enabled on the system with the following command:  
  
     $ systemctl status ufw.service | grep -i "active:" 
     Active: active (exited) since Thu 2022-1
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must have an application firewall enabled.')
