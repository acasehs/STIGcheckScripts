...
A group authenticator is a generic account used by mul
...
def perform_check(config):
    ...
    # Check Content: Verify Ubuntu 22.04 LTS prevents direct logins to the root account by using the following command:  
  
     $ sudo passwd -S root  
     root L 08/09/2022 0 99999 7 -1 
  
If the output does not cont
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must prevent direct login into the root account.')
