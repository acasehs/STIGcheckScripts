...
def perform_check(config):
    ...
    # Check Content: Verify the system commands directories are owned by "root" by using the following command:  
  
     $ sudo find /bin /sbin /usr/bin /usr/sbin /usr/local/bin /usr/local/sbin ! -user root -type d -exec
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must have directories that contain system commands owned by "root".')
