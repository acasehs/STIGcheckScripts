...
Description:
Control of program execution is a mechanism used to prevent execution of unauthorized programs. Some operating systems may provide a capability that runs counter to the mission or provides users with 
...
def perform_check(config):
    ...
    # Check Content: Verify Ubuntu 22.04 LTS has the "apparmor" package installed by using the following command: 
  
     $ dpkg -l | grep apparmor  
     ii     apparmor     3.0.4-2ubuntu2.3     amd64     user-space par
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must have the "apparmor" package installed.')
