...
def perform_check(config):
    ...
    # Check Content: Verify the systemwide library directories "/lib", "/lib64", and "/usr/lib" are group-owned by "root" by using the following command:  
  
     $ sudo find /lib /usr/lib /lib64 ! -group root -type d -e
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS library directories must be group-owned by "root".')
