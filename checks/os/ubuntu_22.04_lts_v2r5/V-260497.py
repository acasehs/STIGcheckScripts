...
def perform_check(config):
    ...
    # Check Content: Verify the systemwide shared library directories "/lib", "/lib64", and "/usr/lib" are owned by "root" by using the following command:  
  
     $ sudo find /lib /usr/lib /lib64 ! -user root -type d -e
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS library directories must be owned by "root".')
