...
Audit information includes all information (e.g., audit reco
...
def perform_check(config):
    ...
    # Check Content: Verify that the audit log files have a mode of "600" or less permissive.  
  
Determine where the audit logs are stored by using the following command:  
  
     $ sudo grep -iw log_file /etc/audit/au
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must be configured so that audit log files are not read- or write-accessible by unauthorized users.')
