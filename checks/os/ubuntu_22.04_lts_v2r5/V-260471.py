...
Description:
If auditing is enabled late in the startup process, the actions of some startup processes may not be audited. Some audit systems also maintain state information only available if auditing is enabled b
...
def perform_check(config):
    ...
    # Check Content: Verify that Ubuntu 22.04 LTS enables auditing at system startup in grub by using the following command:
 
$ sudo grep "^\s*linux" /boot/grub/grub.cfg 

linux        /boot/vmlinuz-5.4.0-31-generic root
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must initiate session audits at system startup.')
