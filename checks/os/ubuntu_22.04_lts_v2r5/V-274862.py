...
Description:
Any script or executable called by cron as root or by any privileged user must be owned by that user, must have the permissions 755 or more restrictive, and should have no extended rights that allow a
...
def perform_check(config):
    ...
    # Check Content: Verify that Ubuntu 22.04 LTS is configured to audit the execution of any system call made by cron as root or as any privileged user.
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must audit any script or executable called by cron as root or by any privileged user.')
