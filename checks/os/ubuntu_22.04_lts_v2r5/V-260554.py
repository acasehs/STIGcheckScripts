...
Description:
Terminating an idle interactive command shell user session within a short time period reduces the window of opportunity for unauthorized personnel to take control of it when left unattended in a virtu
...
def perform_check(config):
    ...
    # Check Content: Verify Ubuntu 22.04 LTS is configured to automatically exit interactive command shell user sessions after 15 minutes of inactivity or less by using the following command: 
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must automatically exit interactive command shell user sessions after 15 minutes of inactivity.')
