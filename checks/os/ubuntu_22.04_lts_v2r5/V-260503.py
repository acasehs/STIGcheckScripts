...
def perform_check(config):
    ...
    # Check Content: Verify the /run/log/journal and /var/log/journal files are owned by "root" by using the following command: 
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must configure the files used by the system journal to be owned by "root".')
