...
Description:
If Ubuntu 22.04 LTS were to allow any user to make changes to software libraries, then those changes might be implemented without undergoing the appropriate testing and approvals that are part of a ro
...
def perform_check(config):
    ...
    # Check Content: Verify the system commands contained in the following directories are group-owned by "root" or a required system account by using the following command:  
  
     $ sudo find -L /bin /sbin /usr/bin /u
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must have system commands group-owned by "root" or a system account.')
