...
def perform_check(config):
    ...
    # Check Content: Verify the group owner of newly created audit logs is "root" by using the following command:  
 
     $ sudo grep -iw log_group /etc/audit/auditd.conf 
     log_group = root 
 
If "log_group" is not s
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must permit only authorized groups ownership of the audit log files.')
