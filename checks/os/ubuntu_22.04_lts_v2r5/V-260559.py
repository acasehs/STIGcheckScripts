...
Security functions are the hardware, software, an
...
def perform_check(config):
    ...
    # Check Content: Verify the sudo group has only members who require access to security functions by using the following command:   
  
     $ grep sudo /etc/group  
     sudo:x:27:<username> 
  
If the sudo group cont
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must ensure only users who need access to security functions are part of sudo group.')
