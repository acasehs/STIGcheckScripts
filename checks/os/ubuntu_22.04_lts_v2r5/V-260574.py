...
DOD has mandated the use of the common access card (CAC) to support identity management and per
...
def perform_check(config):
    ...
    # Check Content: Verify the "opensc-pcks11" package is installed on the system by using the following command:  
  
     $ dpkg -l | grep opensc-pkcs11 
     ii     opensc-pkcs11:amd64     0.22.0-1Ubuntu2     amd64   
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must accept personal identity verification (PIV) credentials.')
