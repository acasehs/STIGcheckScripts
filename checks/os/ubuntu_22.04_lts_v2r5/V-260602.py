...
def perform_check(config):
    ...
    # Check Content: Verify that "/etc/audit/audit.rules", "/etc/audit/auditd.conf", and "/etc/audit/rules.d/*" files are owned by root account by using the following command:  
  
     $ sudo ls -al /etc/audit/audit.rule
...
def main():
    parser = argparse.ArgumentParser(description='Ubuntu 22.04 LTS must permit only authorized accounts to own the audit configuration files.')
