...
#
# Check Content:
#     Verify the operating system disables the ability to load the ATM protocol kernel module and ensure that the ATM protocol kernel module is disabled with the following command:  $ sudo grep -r atm /etc/modprobe.d/* | grep "blacklist"  /etc/modprobe.d/blacklist.conf:install atm /bin/false /etc/modprobe...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230494"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must disable the asynchronous transfer mode (ATM) protocol.",
  "status": "$STATUS",
