...
#
# Check Content:
#     Verify RHEL 8 generates an audit record when successful/unsuccessful modifications to the "lastlog" file by performing the following command to check the file system rules in "/etc/audit/audit.rules":  $ sudo grep -w lastlog /etc/audit/audit.rules  -w /var/log/lastlog -p wa -k logins  If the command...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230467"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "Successful/unsuccessful modifications to the lastlog file in RHEL 8 must generate an audit record.",
  "status": "$STATUS",
