...
#
# Check Content:
#     Verify the operating system has the packages required for offloading audit logs installed with the following commands:  $ sudo yum list installed rsyslog  rsyslog.x86_64          8.1911.0-3.el8          @AppStream  If the "rsyslog" package is not installed, ask the administrator to indicate how audi...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230477"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must have the packages required for offloading audit logs installed.",
  "status": "$STATUS",
