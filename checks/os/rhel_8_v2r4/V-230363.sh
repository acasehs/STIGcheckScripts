...
#
# Check Content:
#     Verify the value of the "difok" option with the following command:  $ sudo grep -r difok /etc/security/pwquality.conf*  /etc/security/pwquality.conf:difok = 8  If the value of "difok" is set to less than "8" or is commented out, this is a finding. If conflicting results are returned, this is a findi...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230363"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must require the change of at least 8 characters when passwords are changed.",
  "status": "$STATUS",
