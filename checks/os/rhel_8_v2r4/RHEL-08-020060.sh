...
The session lock is implemented at the point where session activity can be determined. Rather than be forced to wait for a period of time to expire before the user session can be locked, RHEL 8 needs to provide users with the ability to manually invoke a session lock so users c
#
# Check Content:
...
main() {
    ...
    echo "Rule: RHEL 8 must automatically lock graphical user sessions after 15 minutes of inactivity."
