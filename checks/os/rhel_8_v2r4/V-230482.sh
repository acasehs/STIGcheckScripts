...
#
# Check Content:
#     Verify the operating system authenticates the remote logging server for off-loading audit logs with the following command:  $ sudo grep -i '$ActionSendStreamDriverAuthMode' /etc/rsyslog.conf /etc/rsyslog.d/*.conf  /etc/rsyslog.conf:$ActionSendStreamDriverAuthMode x509/name  If the value of the "$Act...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230482"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must authenticate the remote logging server for off-loading audit logs.",
  "status": "$STATUS",
