...
#
# Check Content:
#     Verify that "/etc/sudoers" has no occurrences of "!authenticate".  Check that the "/etc/sudoers" file has no occurrences of "!authenticate" by running the following command:  $ sudo egrep -iR '!authenticate' /etc/sudoers /etc/sudoers.d/  If any occurrences of "!authenticate" return from the command,...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230272"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must require users to reauthenticate for privilege escalation.",
  "status": "$STATUS",
