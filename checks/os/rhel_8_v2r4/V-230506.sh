...
#
# Check Content:
#     Verify there are no wireless interfaces configured on the system with the following command:  Note: This requirement is Not Applicable for systems that do not have physical wireless network radios.  $ sudo nmcli device status  DEVICE                    TYPE            STATE                    CONNEC...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230506"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 wireless network adapters must be disabled.",
  "status": "$STATUS",
