...
#
# Check Content:
#     Verify the SSH client is configured to use only ciphers employing FIPS 140-3 approved algorithms._x000D_ _x000D_ To verify the Ciphers in the systemwide SSH configuration file, use the following command:_x000D_ _x000D_ $ sudo grep -i ciphers /etc/crypto-policies/back-ends/openssh.config _x000D_ -oCi...
#
# Exit Codes:
...
echo "TODO: Implement check for V-272483"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 SSH client must be configured to use only ciphers employing FIPS 140-3 validated cryptographic hash algorithms.",
  "status": "$STATUS",
