...
#
# Check Content:
#     Verify the operating system disables the ability to load the SCTP kernel module and ensure that SCTP is disabled with the following command:  $ sudo grep -r sctp /etc/modprobe.d/* | grep "blacklist"  /etc/modprobe.d/blacklist.conf:install sctp /bin/false /etc/modprobe.d/blacklist.conf:blacklist sctp...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230496"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must disable the stream control transmission protocol (SCTP).",
  "status": "$STATUS",
