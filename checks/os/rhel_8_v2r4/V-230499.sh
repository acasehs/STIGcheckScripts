...
#
# Check Content:
#     Verify the operating system disables the ability to load the firewire-core kernel module and ensure that the firewire-core kernel module is disabled with the following command:  $ sudo grep -r firewire-core /etc/modprobe.d/* | grep "blacklist"  /etc/modprobe.d/blacklist.conf:install firewire-core /b...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230499"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must disable IEEE 1394 (FireWire) Support.",
  "status": "$STATUS",
