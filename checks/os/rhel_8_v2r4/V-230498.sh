...
#
# Check Content:
#     Verify the operating system disables the ability to load the cramfs kernel module and ensure that the cramfs kernel module is disabled with the following command:  $ sudo grep -r cramfs /etc/modprobe.d/* | grep "blacklist"  /etc/modprobe.d/blacklist.conf:install cramfs /bin/false /etc/modprobe.d/bla...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230498"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must disable mounting of cramfs.",
  "status": "$STATUS",
