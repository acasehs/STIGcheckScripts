...
In RHEL 8.2 the \"/etc/security/faillock.conf\" file was incorporated to centralize the configuration of the pam_faillock.so module. Also introduced is a \"local_users_only\" option that will only track failed user authentication attempts for local users in /etc/passwd and ignore central
#
# Check Content:
...
main() {
    ...
    echo "Rule: RHEL 8 must configure the use of the pam_faillock.so module in the /etc/pam.d/system-auth file."
