...
#
# Check Content:
#     Verify RHEL 8 is not configured to reboot the system when Ctrl-Alt-Delete is pressed seven times within two seconds with the following command:  $ sudo grep -i ctrl /etc/systemd/system.conf  CtrlAltDelBurstAction=none  If the "CtrlAltDelBurstAction" is not set to "none", commented out, or is missing...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230531"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "The systemd Ctrl-Alt-Delete burst key sequence in RHEL 8 must be disabled.",
  "status": "$STATUS",
