...
#
# Check Content:
#     Verify the operating system encrypts audit records off-loaded onto a different system or media from the system being audited with the following commands:  $ sudo grep -i '$DefaultNetstreamDriver' /etc/rsyslog.conf /etc/rsyslog.d/*.conf  /etc/rsyslog.conf:$DefaultNetstreamDriver gtls  If the value of...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230481"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must encrypt the transfer of audit records off-loaded onto a different system or media from the system being audited.",
  "status": "$STATUS",
