...
#
# Check Content:
#     Verify the audit logs are group-owned by "root". First determine where the audit logs are stored with the following command:  $ sudo grep -iw log_file /etc/audit/auditd.conf  log_file = /var/log/audit/audit.log  Using the location of the audit log file, determine if the audit log is group-owned by "...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230398"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 audit logs must be group-owned by root to prevent unauthorized read access.",
  "status": "$STATUS",
