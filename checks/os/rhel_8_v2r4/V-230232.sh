...
#
# Check Content:
#     Confirm that the interactive user account passwords are using a strong password hash with the following command:  $ sudo cut -d: -f2 /etc/shadow  $6$kcOnRq/5$NUEYPuyL.wghQwWssXRcLRFiiru7f5JPV6GaJhNC2aK5F3PZpE/BCCtwrxRc/AInKMNX3CdMw11m9STiql12f/  Password hashes "!" or "*" indicate inactive accounts ...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230232"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must employ FIPS 140-2 approved cryptographic hashing algorithms for all stored passwords.",
  "status": "$STATUS",
