...
auth required pam_faillock.so preauth dir=/var
#
# Exit Codes:
...
main() {
    ...
    echo "Rule: RHEL 8 must include root when automatically locking an account until the locked account is released by an administrator when three unsuccessful logon attempts occur during a 15-minute time period."
