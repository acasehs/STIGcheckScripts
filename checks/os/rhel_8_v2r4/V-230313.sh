...
#
# Check Content:
#     Verify the operating system disables core dumps for all users by issuing the following command:  $ sudo grep -r -s '^[^#].*core' /etc/security/limits.conf /etc/security/limits.d/*.conf  /etc/security/limits.d/core_dumps.conf:* hard core 0  This can be set as a global domain (with the * wildcard) but...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230313"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must disable core dumps for all users.",
  "status": "$STATUS",
