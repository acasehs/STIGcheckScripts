...
#
# Check Content:
#     Verify the operating system disables the ability to load the TIPC protocol kernel module and ensure that TIPC is disabled with the following command:  $ sudo grep -r tipc /etc/modprobe.d/* | grep "blacklist"  /etc/modprobe.d/blacklist.conf:install tipc /bin/false /etc/modprobe.d/blacklist.conf:black...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230497"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must disable the transparent inter-process communication (TIPC) protocol.",
  "status": "$STATUS",
