...
#
# Check Content:
#     Verify file systems that are used for removable media are mounted with the "nodev" option with the following command:  $ sudo more /etc/fstab  UUID=2bc871e4-e2a3-4f29-9ece-3be60c835222 /mnt/usbflash vfat noauto,owner,ro,nosuid,nodev,noexec 0 0  If a file system found in "/etc/fstab" refers to remova...
#
# Exit Codes:
...
echo "TODO: Implement check for V-230303"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must prevent special devices on file systems that are used with removable media.",
  "status": "$STATUS",
