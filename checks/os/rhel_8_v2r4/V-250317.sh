...
#
# Check Content:
#     Verify RHEL 8 is not performing IPv4 packet forwarding, unless the system is a router.  Check that IPv4 forwarding is disabled using the following command:  $ sudo sysctl net.ipv4.conf.all.forwarding  net.ipv4.conf.all.forwarding = 0 If the IPv4 forwarding value is not "0" and is not documented with...
#
# Exit Codes:
...
echo "TODO: Implement check for V-250317"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must not enable IPv4 packet forwarding unless the system is a router.",
  "status": "$STATUS",
