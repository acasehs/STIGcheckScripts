...
#
# Check Content:
#     Verify the pam_faillock.so module is present and is listed before the pam.unix.so line in the "/etc/pam.d/system-auth" file: Note: The first field in the output is the line number of the entry  $ sudo grep -E -n 'pam_faillock.so|pam_unix.so' /etc/pam.d/system-auth  7:auth required pam_faillock.so pr...
#
# Exit Codes:
...
echo "TODO: Implement check for V-244533"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "RHEL 8 must configure the use of the pam_faillock.so module in the /etc/pam.d/system-auth file.",
  "status": "$STATUS",
