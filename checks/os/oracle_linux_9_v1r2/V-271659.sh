...
#
# Check Content:
#     Verify that OL 9 is configured to mount /var/log with the nosuid option.  Verify "/var/log" is mounted with the "nosuid" option:  $ mount | grep /var/log /dev/mapper/ol-var-log on /var/log type xfs (rw,nodev,nosuid,noexec,seclabel)  If the "/var/log" file system is mounted without the "nosuid" optio...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271659"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 must mount /var/log with the nosuid option.",
  "status": "$STATUS",
