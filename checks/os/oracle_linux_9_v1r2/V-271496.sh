...
#
# Check Content:
#     Verify that OL 9 has the package installed with the following command:  $ dnf list --installed aide Installed Packages aide.x86_64                                         0.16-100.el9                                          @ol9_appstream  If AIDE is not installed, ask the system administrator (SA)...
#
# Exit Codes:
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 must have the Advanced Intrusion Detection Environment (AIDE) package installed.",
  "status": "$STATUS",
