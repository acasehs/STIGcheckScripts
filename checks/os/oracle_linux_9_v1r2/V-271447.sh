...
#
# Check Content:
#     Verify that OL 9 disables the ability to load the tipc kernel module with the following command:  $ grep -r tipc /etc/modprobe.conf /etc/modprobe.d/*  blacklist tipc  If the command does not return any output, or the line is commented out, and use of TIPC is not documented with the information syste...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271447"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 must disable the Transparent Inter Process Communication (TIPC) kernel module.",
  "status": "$STATUS",
