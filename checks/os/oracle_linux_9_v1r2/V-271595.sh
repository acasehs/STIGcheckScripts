...
#
# Check Content:
#     Verify that OL 9 generates an audit record for all uses of the umount2 system call with the following commands:  $ sudo grep "umount2" /etc/audit/audit.rules   $ sudo sh -c 'grep "umount2" /etc/audit/audit.rules'   If the system is configured to audit this activity, it will return a line.  If no lin...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271595"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 must be configured so that successful/unsuccessful uses of the umount2 system call generate an audit record.",
  "status": "$STATUS",
