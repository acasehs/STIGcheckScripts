...
#
# Check Content:
#     Verify that OL 9 TFTP daemon is configured to operate in secure mode.  Check if a TFTP server is installed with the following command:  $ sudo dnf list --installed tftp-server Installed Packages tftp-server.x86_64                                       5.2-38.el9                                      ...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271764"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 Trivial File Transfer Protocol (TFTP) daemon must be configured to operate in secure mode if the TFTP server is required. ",
  "status": "$STATUS",
