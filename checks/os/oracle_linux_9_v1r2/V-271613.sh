...
#
# Check Content:
#     Verify that OL 9 enforces password complexity by requiring at least one uppercase character.  Check the value for "ucredit" with the following command:  $ grep ucredit /etc/security/pwquality.conf /etc/security/pwquality.conf.d/*.conf  ucredit = -1   If the value of "ucredit" is a positive number or...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271613"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 must enforce password complexity by requiring that at least one uppercase character be used.",
  "status": "$STATUS",
