...
#
# Check Content:
#     Verify that OL 9 disables the ability to load the sctp kernel module with the following command:  $ grep -r sctp /etc/modprobe.conf /etc/modprobe.d/*  blacklist sctp  If the command does not return any output, or the line is commented out, and use of sctp is not documented with the information syste...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271446"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 must disable the Stream Control Transmission Protocol (SCTP) kernel module.",
  "status": "$STATUS",
