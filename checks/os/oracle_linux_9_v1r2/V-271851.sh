...
#
# Check Content:
#     Verify that OL 9 monitors all remote access methods.  Check that remote access methods are being logged by running the following command:  $ grep -rE '(auth.\*|authpriv.\*|daemon.\*)' /etc/rsyslog.conf authpriv.*                                              /var/log/secure   If "auth.*", "authpriv.*...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271851"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 remote access methods must be monitored.",
  "status": "$STATUS",
