...
#
# Check Content:
#     Verify that OL 9 is not configured to reboot the system when Ctrl-Alt-Delete is pressed with the following command:  $ systemctl status ctrl-alt-del.target ctrl-alt-del.target Loaded: masked (Reason: Unit ctrl-alt-del.target is masked.) Active: inactive (dead)  If the "ctrl-alt-del.target" is loaded...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271752"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 must be configured so that the x86 Ctrl-Alt-Delete key sequence is disabled.",
  "status": "$STATUS",
