...
#
# Check Content:
#     If no NFS mounts are configured, this requirement is Not Applicable.  Verify that OL 9 has the "nodev" option configured for all NFS mounts with the following command:  $ cat /etc/fstab | grep nfs 192.168.22.2:/mnt/export /data nfs4 rw,nosuid,nodev,noexec,sync,soft,sec=krb5:krb5i:krb5p  If the syste...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271641"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 must prevent special devices on file systems that are imported via Network File System (NFS).",
  "status": "$STATUS",
