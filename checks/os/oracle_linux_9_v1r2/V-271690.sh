...
#
# Check Content:
#     This requirement assumes the use of the OL 9 default graphical user interfaceâ€”the GNOME desktop environment. If the system does not have any graphical user interface installed, this requirement is Not Applicable.  Verify that OL 9 enables a user's session lock until that user reestablishes access ...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271690"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 must be able to directly initiate a session lock for all connection types using smart card when the smart card is removed.",
  "status": "$STATUS",
