...
Rule ID: SV-271703r1091821

Description:
    SSH provides several logging levels with varying amounts of verbosity. "DEBUG" is specifically not recommended other than strictly for debugging SSH communications since it provides so much data that it is difficult to identify important security information. "INFO" or "VERBOSE" level is the basic l...

Check Content:
    Verify that OL 9 logs SSH connection attempts and failures to the server.  Check what the SSH daemon's "LogLevel" option is set to with the following command:  $ sudo /usr/sbin/sshd -dd 2>&1 | awk '/filename/ {print $4}' | tr -d '\r' | tr '\n' ' ' | xargs sudo grep -iH '^\s*loglevel' LogLevel VERBOS...
...
# Check metadata
VULN_ID = "V-271703"
STIG_ID = "OL09-00-002340"
SEVERITY = "medium"
RULE_TITLE = """OL 9 must log SSH connection attempts and failures to the server."""
