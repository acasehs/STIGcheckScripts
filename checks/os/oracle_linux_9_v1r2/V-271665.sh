...
#
# Check Content:
#     Verify that OL 9 is configured to mount /var/tmp with the nosuid option.  Verify "/var/tmp" is mounted with the "nosuid" option:  $ mount | grep /var/tmp /dev/mapper/ol-var-tmp on /var/tmp type xfs (rw,nodev,nosuid,noexec,seclabel)  If the "/var/tmp" file system is mounted without the "nosuid" optio...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271665"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 must mount /var/tmp with the nosuid option.",
  "status": "$STATUS",
