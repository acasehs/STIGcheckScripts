...
#
# Check Content:
#     Verify that OL 9 is configured to audit the execution of the chmod, fchmod, and fchmodat system calls with the following command:  $ sudo auditctl -l | grep chmod -a always,exit -F arch=b32 -S chmod,fchmod,fchmodat -F auid>=1000 -F auid!=unset -k perm_mod -a always,exit -F arch=b64 -S chmod,fchmod,f...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271555"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 must audit all uses of the chmod, fchmod, and fchmodat system calls.",
  "status": "$STATUS",
