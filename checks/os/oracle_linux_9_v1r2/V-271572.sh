...
#
# Check Content:
#     Verify that OL 9 generates audit records for all account creations, modifications, disabling, and termination events that affect /var/log/tallylog with the following command:  $ sudo auditctl -l | grep /var/log/tallylog -wÂ /var/log/tallylogÂ -pÂ waÂ -kÂ logins  If the command does not return a line...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271572"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 must generate audit records for all account creations, modifications, disabling, and termination events that affect /var/log/tallylog.",
  "status": "$STATUS",
