...
#
# Check Content:
#     Verify that OL 9 system-wide shared library directories are group-owned by "root" with the following command:  $ sudo find /lib /lib64 /usr/lib /usr/lib64 ! -group root -type d -exec stat -c "%n %G" '{}' \;  If any system-wide shared library directory is returned and is not group-owned by a required...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271786"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 library directories must be group-owned by root or a system account.",
  "status": "$STATUS",
