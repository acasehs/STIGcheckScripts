...
#
# Check Content:
#     Verify that OL 9 system commands contained in the following directories are owned by "root" with the following command:  $ sudo find -L /bin /sbin /usr/bin /usr/sbin /usr/libexec /usr/local/bin /usr/local/sbin ! -user root -exec ls -l {} \;  If any system commands are found to not be owned by root, ...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271774"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 system commands must be owned by root.",
  "status": "$STATUS",
