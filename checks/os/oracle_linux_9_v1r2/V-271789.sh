...
#
# Check Content:
#     Verify that OL 9 system-wide shared library files are group-owned by "root" with the following command:  $ sudo find -L /lib /lib64 /usr/lib /usr/lib64 ! -group root -exec ls -l {} \;  If any system-wide shared library file is returned and is not group-owned by a required system account, this is a f...
#
# Exit Codes:
...
echo "TODO: Implement check for V-271789"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 9 library files must be group-owned by root or a system account.",
  "status": "$STATUS",
