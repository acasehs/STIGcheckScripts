...
#
# Check Content:
#     Verify OL 8 generates an audit event for any use of the "pam_timestamp_check" command by running the following command to check the file system rules in "/etc/audit/audit.rules":    $ sudo grep -w pam_timestamp_check /etc/audit/audit.rules    -a always,exit -F path=/usr/sbin/pam_timestamp_check -F p...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248771"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must generate audit records for any use of the "pam_timestamp_check" command.",
  "status": "$STATUS",
