...
#
# Check Content:
#     Verify OL 8 generates an audit event for any use of the "passwd" command by running the following command to check the file system rules in "/etc/audit/audit.rules":    $ sudo grep -w passwd /etc/audit/audit.rules    -a always,exit -F path=/usr/bin/passwd -F perm=x -F auid>=1000 -F auid!=unset -k pr...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248757"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must generate audit records for any use of the "passwd" command.",
  "status": "$STATUS",
