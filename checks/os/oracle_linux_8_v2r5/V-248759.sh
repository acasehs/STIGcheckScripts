...
#
# Check Content:
#     Verify OL 8 generates an audit event for any use of the "umount" command by running the following command to check the file system rules in "/etc/audit/audit.rules":    $ sudo grep -w /usr/bin/umount /etc/audit/audit.rules    -a always,exit -F path=/usr/bin/umount -F perm=x -F auid>=1000 -F auid!=un...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248759"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must generate audit records for any use of the "umount" command.",
  "status": "$STATUS",
