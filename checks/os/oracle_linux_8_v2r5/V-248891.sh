...
#
# Check Content:
#     Verify OL 8 restricts exposed kernel pointer addresses access with the following commands:   $ sudo sysctl kernel.kptr_restrict    kernel.kptr_restrict = 1    If the returned line does not have a value of "1" or "2" or a line is not returned, this is a finding.  Check that the configuration files ar...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248891"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must restrict exposed kernel pointer addresses access.",
  "status": "$STATUS",
