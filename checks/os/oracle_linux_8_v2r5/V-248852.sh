...
#
# Check Content:
#     Verify "/var/log" is mounted with the "noexec" option:  $ sudo mount | grep /var/log  /dev/mapper/ol-var_log on /var/log type xfs (rw,nodev,nosuid,noexec,seclabel)  Verify that the "noexec" option is configured for /var/log:  $ sudo cat /etc/fstab | grep /var/log  /dev/mapper/ol-var_log /var/log xfs...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248852"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must mount "/var/log" with the "noexec" option.",
  "status": "$STATUS",
