...
#
# Check Content:
#     Verify OL 8 generates an audit record for any use of the "chmod","fchmod", and "fchmodat" syscalls by running the following command to check the file system rules in "/etc/audit/audit.rules":    $ sudo grep chmod /etc/audit/audit.rules    -a always,exit -F arch=b32 -S chmod,fchmod,fchmodat -F auid>=...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248791"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must generate audit records for any use of the "chmod", "fchmod", and "fchmodat" system calls.",
  "status": "$STATUS",
