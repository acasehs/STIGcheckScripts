...
#
# Check Content:
#     Verify "/var/log" is mounted with the "nodev" option:  $ sudo mount | grep /var/log  /dev/mapper/ol-var_log on /var/log type xfs (rw,nodev,nosuid,noexec,seclabel)  Verify that the "nodev" option is configured for /var/log:  $ sudo cat /etc/fstab | grep /var/log  /dev/mapper/ol-var_log /var/log xfs d...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248850"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must mount "/var/log" with the "nodev" option.",
  "status": "$STATUS",
