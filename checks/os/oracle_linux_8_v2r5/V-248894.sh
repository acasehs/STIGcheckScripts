...
#
# Check Content:
#     Verify OL 8 enables hardening for the BPF JIT with the following commands:  $ sudo sysctl net.core.bpf_jit_harden  net.core.bpf_jit_harden = 2  If the returned line does not have a value of "2", or a line is not returned, this is a finding.  Check that the configuration files are present to enable t...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248894"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must enable hardening for the Berkeley Packet Filter Just-in-time compiler.",
  "status": "$STATUS",
