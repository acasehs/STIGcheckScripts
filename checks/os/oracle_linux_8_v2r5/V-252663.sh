...
#
# Check Content:
#     Verify that the system is configured to boot to the command line:  $ systemctl get-default multi-user.target  If the system default target is not set to "multi-user.target" and the Information System Security Officer (ISSO) lacks a documented requirement for a graphical user interface, this is a fin...
#
# Exit Codes:
...
echo "TODO: Implement check for V-252663"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "The graphical display manager must not be the default target on OL 8 unless approved.",
  "status": "$STATUS",
