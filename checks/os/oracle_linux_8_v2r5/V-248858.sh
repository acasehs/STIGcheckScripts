...
#
# Check Content:
#     Verify "/var/tmp" is mounted with the "noexec" option:  $ sudo mount | grep /var/tmp  /dev/mapper/ol-var_tmp on /var/tmp type xfs (rw,nodev,nosuid,noexec,seclabel)  Verify that the "noexec" option is configured for /var/tmp:  $ sudo cat /etc/fstab | grep /var/tmp  /dev/mapper/ol-var_tmp /var/tmp xfs...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248858"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must mount "/var/tmp" with the "noexec" option.",
  "status": "$STATUS",
