...
#
# Check Content:
#     Verify OL 8 generates audit records for all account creations events that affect "/etc/group".    Check the auditing rules in "/etc/audit/audit.rules" with the following command:    $ sudo grep /etc/group /etc/audit/audit.rules    -w /etc/group -p wa -k identity    If the command does not return a l...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248744"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must generate audit records for all account creation events that affect "/etc/group".",
  "status": "$STATUS",
