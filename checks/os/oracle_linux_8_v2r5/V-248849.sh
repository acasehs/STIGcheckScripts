...
#
# Check Content:
#     Verify "/tmp" is mounted with the "noexec" option:  $ sudo mount | grep /tmp  /dev/mapper/ol-tmp on /tmp type xfs (rw,nodev,nosuid,noexec,seclabel)  Verify that the "noexec" option is configured for /tmp:  $ sudo cat /etc/fstab | grep /tmp  /dev/mapper/ol-tmp /tmp xfs defaults,nodev,nosuid,noexec 0 ...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248849"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must mount "/tmp" with the "noexec" option.",
  "status": "$STATUS",
