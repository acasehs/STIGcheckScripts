...
#
# Check Content:
#     Verify OL 8 generates audit records for all account creations, modifications, disabling, and termination events that affect "/etc/shadow".    Check the auditing rules in "/etc/audit/audit.rules" with the following command:    $ sudo grep /etc/shadow /etc/audit/audit.rules    -w /etc/shadow -p wa -k ...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248740"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must generate audit records for all account creation events that affect "/etc/shadow".",
  "status": "$STATUS",
