...
#
# Check Content:
#     Verify OL 8 generates an audit event for any use of the "chage" command by running the following command to check the file system rules in "/etc/audit/audit.rules":    $ sudo grep -w chage /etc/audit/audit.rules    -a always,exit -F path=/usr/bin/chage -F perm=x -F auid>=1000 -F auid!=unset -k privi...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248753"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must generate audit records for any use of the "chage" command.",
  "status": "$STATUS",
