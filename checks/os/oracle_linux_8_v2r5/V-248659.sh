...
#
# Check Content:
#     Note: This check applies to OL versions 8.2 or newer. If the system is OL version 8.0 or 8.1, this check is not applicable.    Verify the "/etc/security/faillock.conf" file is configured use a non-default faillock directory to ensure contents persist after reboot:    $ sudo grep 'dir =' /etc/securit...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248659"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 systems, versions 8.2 and above, must ensure account lockouts persist.",
  "status": "$STATUS",
