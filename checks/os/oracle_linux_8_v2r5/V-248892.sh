...
#
# Check Content:
#     Verify OL 8 disables the use of user namespaces with the following commands.    Note: If unprivileged user namespaces or "rootless" containers are in use, this requirement is not applicable.        $ sudo sysctl user.max_user_namespaces       user.max_user_namespaces = 0    If the returned line does...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248892"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must disable the use of user namespaces.",
  "status": "$STATUS",
