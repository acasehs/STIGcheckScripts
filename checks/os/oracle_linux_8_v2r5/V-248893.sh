...
#
# Check Content:
#     Verify OL 8 uses reverse path filtering on all IPv4 interfaces with the following commands:    $ sudo sysctl net.ipv4.conf.all.rp_filter    net.ipv4.conf.all.rp_filter = 1    If the returned line does not have a value of "1" or "2" or a line is not returned, this is a finding.  Check that the config...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248893"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must use reverse path filtering on all IPv4 interfaces.",
  "status": "$STATUS",
