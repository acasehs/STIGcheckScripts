...
#
# Check Content:
#     Verify the operating system disables the ability to load the "sctp" kernel module.          $ sudo grep -r sctp /etc/modprobe.d/* | grep -i "/bin/false" | grep -v "^#"       install sctp /bin/false   If the command does not return any output or the line is commented out, and use of SCTP is not docum...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248831"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must not have the stream control transmission protocol (SCTP) kernel module installed if not required for operational support.",
  "status": "$STATUS",
