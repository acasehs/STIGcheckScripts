...
#
# Check Content:
#     Verify that "/etc/sudoers" has no occurrences of "NOPASSWD".  Check that the "/etc/sudoers" file has no occurrences of "NOPASSWD" by running the following command:  $ sudo grep -iR 'NOPASSWD' /etc/sudoers /etc/sudoers.d/  %admin ALL=(ALL) NOPASSWD: ALL  If any occurrences of "NOPASSWD" are returned ...
#
# Exit Codes:
...
echo "TODO: Implement check for V-248581"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OL 8 must require users to provide a password for privilege escalation.",
  "status": "$STATUS",
