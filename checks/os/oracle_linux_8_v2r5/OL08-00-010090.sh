...
When there is a chain of trust, usually the top entity to be trusted becomes the trust anchor; it can be, for example, a Certification Authority (CA). A ce
#
# Check Content:
...
If the root ca file is not a DOD-issued certificate with a valid date installed in the \"/etc/sssd/pki/sssd_auth_ca_db.pem
#
# Exit Codes:
...
main() {
    ...
    echo "Rule: OL 8, for PKI-based authentication, must validate certificates by constructing a certification path (which includes status information) to an accepted trust anchor."
