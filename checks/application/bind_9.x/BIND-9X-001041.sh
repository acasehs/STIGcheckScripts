...
#
# Check Content:
#     Verify that the BIND 9.x server is configured to send audit logs to a local log file._x000D_ _x000D_ NOTE: syslog and local file channel must be defined for every defined category._x000D_ _x000D_ Inspect the "named.conf" file for the following:_x000D_ _x000D_ logging {_x000D_ channel local_file_chan...
#
# Exit Codes:
...
echo "TODO: Implement BIND 9.x check for BIND-9X-001041"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "The BIND 9.x server implementation must be configured with a channel to send audit records to a local file.",
  "status": "$STATUS",
