...
#
# Check Content:
#     If this is a secondary name server, this is Not Applicable.  On a master name server, verify that the global notify is disabled. The global entry for the name server is under the â€œOptionsâ€ section and notify should be disabled at this section.  Inspect the "named.conf" file for the following:  o...
#
# Exit Codes:
...
echo "TODO: Implement BIND 9.x check for BIND-9X-001057"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "The master servers in a BIND 9.x implementation must notify authorized secondary name servers when zone files are updated.",
  "status": "$STATUS",
...
Severity: ${SEVERITY^^}
================================================================================
Rule: The master servers in a BIND 9.x implementation must notify authorized secondary name servers when zone files are updated.
Status: $STATUS
Timestamp: $TIMESTAMP
