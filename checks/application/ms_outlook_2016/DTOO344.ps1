<#
.SYNOPSIS
    DTOO344 - Outlook Rich Text options must be set for converting to plain text format.
...
.EXAMPLE
    .\DTOO344.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\DTOO344.ps1 -OutputJson
...
.NOTES
    ...
    Registry Check:
    Key: HKCU\Software\Policies\Microsoft\Office\16.0\outlook\options\mail_x000D_
#>
...
$result = @{
    STIG_ID = "DTOO344"
    Rule_Title = "Outlook Rich Text options must be set for converting to plain text format."
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
...
try {
    ...
    $regPath = "HKCU:\Software\Policies\Microsoft\Office\16.0\outlook\options\mail_x000D_"
