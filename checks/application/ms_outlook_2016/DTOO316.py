#!/usr/bin/env python3
"""
DTOO316 - Outlook minimum encryption key length settings must be set.

STIG ID: DTOO316
Rule Title: Outlook minimum encryption key length settings must be set.
Severity: medium
Vuln ID: V-228474
Rule ID: SV-228474r508021
...
Registry Check:
    ...
    Value: MinEncKey
    Type: REG_DWORD
    Expected: a8 (hex) or 168 (decimal)
"""
...
def perform_check(config: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    result = {
        "STIG_ID": "DTOO316",
        "Rule_Title": "Outlook minimum encryption key length settings must be set.",
        "Severity": "medium",
        "Status": "Not_Reviewed",
        "Finding_Details": "",
        "Comments": ""
    ...
    try:
        ...
        expected_value = a8 (hex) or 168 (decimal)
        value_type = "DWORD"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="DTOO316 - Outlook minimum encryption key length settings must be set.",
