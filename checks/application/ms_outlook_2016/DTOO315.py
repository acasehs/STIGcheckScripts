#!/usr/bin/env python3
"""
DTOO315 - Outlook must be configured not to prompt users to choose security settings if default settings fail.

STIG ID: DTOO315
Rule Title: Outlook must be configured not to prompt users to choose security settings if default settings fail.
Severity: medium
Vuln ID: V-228473
Rule ID: SV-228473r508021
...
def perform_check(config: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    result = {
        "STIG_ID": "DTOO315",
        "Rule_Title": "Outlook must be configured not to prompt users to choose security settings if default settings fail.",
        "Severity": "medium",
        "Status": "Not_Reviewed",
        "Finding_Details": "",
        "Comments": ""
    ...
    try:
        ...
        value_name = "ForceDefaultProfile"
        expected_value = 0
        value_type = "DWORD"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="DTOO315 - Outlook must be configured not to prompt users to choose security settings if default settings fail.",
