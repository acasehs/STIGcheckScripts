<#
.SYNOPSIS
    DTOO505 - Scripted Window Security must be enforced in PowerPoint Viewer._x000D_

.DESCRIPTION
    STIG ID: DTOO505
    Rule Title: Scripted Window Security must be enforced in PowerPoint Viewer._x000D_
    Severity: medium
    Vuln ID: V-70691
    Rule ID: SV-85315r1
...
.EXAMPLE
    .\DTOO505.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\DTOO505.ps1 -OutputJson
...
.NOTES
    ...
    Registry Check:
    Key: HKLM\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_WINDOW_RESTRICTIONS_x000D_
#>
...
$result = @{
    STIG_ID = "DTOO505"
    Rule_Title = "Scripted Window Security must be enforced in PowerPoint Viewer._x000D_"
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
...
try {
    ...
    $regPath = "HKLM:\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_WINDOW_RESTRICTIONS_x000D_"
