<#
.SYNOPSIS
    DTOO129 - Links that invoke instances of Internet Explorer from within an Office product must be blocked in PowerPoint._x000D_

.DESCRIPTION
    STIG ID: DTOO129
    Rule Title: Links that invoke instances of Internet Explorer from within an Office product must be blocked in PowerPoint._x000D_
    Severity: medium
    Vuln ID: V-70657
    Rule ID: SV-85279r1
...
.EXAMPLE
    .\DTOO129.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\DTOO129.ps1 -OutputJson
...
$result = @{
    STIG_ID = "DTOO129"
    Rule_Title = "Links that invoke instances of Internet Explorer from within an Office product must be blocked in PowerPoint._x000D_"
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
