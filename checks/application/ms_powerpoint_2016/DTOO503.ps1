<#
.SYNOPSIS
    DTOO503 - The Saved from URL mark must be selected to enforce Internet zone processing in PowerPoint Viewer._x000D_

.DESCRIPTION
    STIG ID: DTOO503
    Rule Title: The Saved from URL mark must be selected to enforce Internet zone processing in PowerPoint Viewer._x000D_
    Severity: medium
    Vuln ID: V-70687
    Rule ID: SV-85311r1
...
.EXAMPLE
    .\DTOO503.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\DTOO503.ps1 -OutputJson
...
.NOTES
    ...
    Registry Check:
    Key: HKLM\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_UNC_SAVEDFILECHECK_x000D_
#>
...
$result = @{
    STIG_ID = "DTOO503"
    Rule_Title = "The Saved from URL mark must be selected to enforce Internet zone processing in PowerPoint Viewer._x000D_"
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
...
try {
    ...
    $regPath = "HKLM:\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_UNC_SAVEDFILECHECK_x000D_"
