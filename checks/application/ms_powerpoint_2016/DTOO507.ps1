<#
.SYNOPSIS
    DTOO507 - Links that invoke instances of Internet Explorer from within an Office product must be blocked in PowerPoint Viewer._x000D_

.DESCRIPTION
    STIG ID: DTOO507
    Rule Title: Links that invoke instances of Internet Explorer from within an Office product must be blocked in PowerPoint Viewer._x000D_
    Severity: medium
    Vuln ID: V-70695
    Rule ID: SV-85319r1
...
.EXAMPLE
    .\DTOO507.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\DTOO507.ps1 -OutputJson
...
.NOTES
    ...
    Registry Check:
    Key: HKLM\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_WEBOC_POPUPMANAGEMENT_x000D_
#>
...
$result = @{
    STIG_ID = "DTOO507"
    Rule_Title = "Links that invoke instances of Internet Explorer from within an Office product must be blocked in PowerPoint Viewer._x000D_"
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
...
try {
    ...
    $regPath = "HKLM:\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_WEBOC_POPUPMANAGEMENT_x000D_"
