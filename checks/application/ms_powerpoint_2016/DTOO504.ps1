<#
.SYNOPSIS
    DTOO504 - Navigation to URLs embedded in Office products must be blocked in PowerPoint Viewer._x000D_

.DESCRIPTION
    STIG ID: DTOO504
    Rule Title: Navigation to URLs embedded in Office products must be blocked in PowerPoint Viewer._x000D_
    Severity: medium
    Vuln ID: V-70689
    Rule ID: SV-85313r1
...
.EXAMPLE
    .\DTOO504.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\DTOO504.ps1 -OutputJson
...
.NOTES
    ...
    Registry Check:
    Key: HKLM\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_VALIDATE_NAVIGATE_URL_x000D_
#>
...
$result = @{
    STIG_ID = "DTOO504"
    Rule_Title = "Navigation to URLs embedded in Office products must be blocked in PowerPoint Viewer._x000D_"
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
...
try {
    ...
    $regPath = "HKLM:\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_VALIDATE_NAVIGATE_URL_x000D_"
