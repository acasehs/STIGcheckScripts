<#
.SYNOPSIS
    DTOO132 - File Downloads must be configured for proper restrictions in PowerPoint._x000D_

.DESCRIPTION
    STIG ID: DTOO132
    Rule Title: File Downloads must be configured for proper restrictions in PowerPoint._x000D_
    Severity: medium
    Vuln ID: V-70661
    Rule ID: SV-85283r1
...
.EXAMPLE
    .\DTOO132.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\DTOO132.ps1 -OutputJson
...
.NOTES
    ...
    Registry Check:
    Key: HKLM\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_RESTRICT_FILEDOWNLOAD_x000D_
#>
...
$result = @{
    STIG_ID = "DTOO132"
    Rule_Title = "File Downloads must be configured for proper restrictions in PowerPoint._x000D_"
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
...
try {
    ...
    $regPath = "HKLM:\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_RESTRICT_FILEDOWNLOAD_x000D_"
