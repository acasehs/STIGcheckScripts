<#
.SYNOPSIS
    DTOO509 - Protection from zone elevation must be enforced in PowerPoint Viewer._x000D_

.DESCRIPTION
    STIG ID: DTOO509
    Rule Title: Protection from zone elevation must be enforced in PowerPoint Viewer._x000D_
    Severity: medium
    Vuln ID: V-70697
    Rule ID: SV-85321r1
...
.EXAMPLE
    .\DTOO509.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\DTOO509.ps1 -OutputJson
...
$result = @{
    STIG_ID = "DTOO509"
    Rule_Title = "Protection from zone elevation must be enforced in PowerPoint Viewer._x000D_"
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
...
try {
    ...
    $regPath = "HKLM:\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_ZONE_ELEVATION_x000D_"
