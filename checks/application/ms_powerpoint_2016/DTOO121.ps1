<#
.SYNOPSIS
    DTOO121 - Files from the Internet zone must be opened in Protected View._x000D_

.DESCRIPTION
    STIG ID: DTOO121
    Rule Title: Files from the Internet zone must be opened in Protected View._x000D_
    Severity: medium
    Vuln ID: V-70651
    Rule ID: SV-85273r1
...
.EXAMPLE
    .\DTOO121.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\DTOO121.ps1 -OutputJson
...
$result = @{
    STIG_ID = "DTOO121"
    Rule_Title = "Files from the Internet zone must be opened in Protected View._x000D_"
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
...
try {
    ...
    $regPath = "HKCU:\Software\Policies\Microsoft\Office\16.0\PowerPoint\security\protectedview_x000D_"
    $regValueName = "DisableInternetFilesInPV"
    $expectedValue = 0
