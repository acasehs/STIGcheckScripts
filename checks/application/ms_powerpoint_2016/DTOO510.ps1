<#
.SYNOPSIS
    DTOO510 - ActiveX Installs must be configured for proper restriction in PowerPoint Viewer._x000D_

.DESCRIPTION
    STIG ID: DTOO510
    Rule Title: ActiveX Installs must be configured for proper restriction in PowerPoint Viewer._x000D_
    Severity: medium
    Vuln ID: V-70699
    Rule ID: SV-85323r1
...
.EXAMPLE
    .\DTOO510.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\DTOO510.ps1 -OutputJson
...
.NOTES
    ...
    Registry Check:
    Key: HKLM\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_RESTRICT_ACTIVEXINSTALL_x000D_
#>
...
$result = @{
    STIG_ID = "DTOO510"
    Rule_Title = "ActiveX Installs must be configured for proper restriction in PowerPoint Viewer._x000D_"
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
...
try {
    ...
    $regPath = "HKLM:\Software\Microsoft\Internet Explorer\Main\FeatureControl\FEATURE_RESTRICT_ACTIVEXINSTALL_x000D_"
