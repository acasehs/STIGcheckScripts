...
.DESCRIPTION
    ...
    Severity: medium
    Rule Title: If file validation fails, files must be opened in Protected view in Po...
...
$RegPath = "HKCU\Software\Policies\Microsoft\Office\16.0\PowerPoint\security\filevalidation"
$RegValue = "for User Configuration >> Administrative Templates >> Microsoft PowerPoint 2016 >> PowerPoint Options >> Security >> Trust Center >> Protected View >> Set document behavior if file validation fails is set to "Enabled: Open in Protected View". Verify the check box for "Allow edit" is not selected."
$ExpectedValue = "1"
