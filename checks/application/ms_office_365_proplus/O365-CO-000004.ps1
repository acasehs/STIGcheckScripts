...
.EXAMPLE
    .\O365-CO-000004.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\O365-CO-000004.ps1 -OutputJson
...
.NOTES
    ...
    Registry Check:
    Key: HKCU\software\policies\microsoft\office\16.0\common\toolbars
    Value: noextensibilitycustomizationfromdocument
    Type: REG_DWORD
    Expected: 1 for all installed Office programs
#>
...
$result = @{
    STIG_ID = "O365-CO-000004"
    Rule_Title = "Custom user interface (UI) code must be blocked from loading in all Office applications."
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
...
try {
    ...
    $regPath = "HKCU:\software\policies\microsoft\office\16.0\common\toolbars"
    $regValueName = "noextensibilitycustomizationfromdocument"
    $expectedValue = 1 for all installed Office programs
