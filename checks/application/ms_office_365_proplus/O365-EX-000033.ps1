...
.DESCRIPTION
    ...
    Severity: medium
    Rule Title: Files failing file validation must be opened in Excel in Protected vie...
...
$RegPath = "HKCU\Software\Policies\Microsoft\Office\16.0\Excel\security\filevalidation"
$RegValue = "for User Configuration >> Administrative Templates >> Microsoft Excel 2016 >> Excel Options >> Security >> Trust Center >> Protected View >> Set document behavior if file validation fails is set to "Enabled: Open in Protected View". Verify the check box for "Allow edit" is not selected."
$ExpectedValue = "1"
