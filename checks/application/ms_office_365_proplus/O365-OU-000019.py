...
def perform_check(config: Dict[str, Any] = None) -> Dict[str, Any]:
    ...
    """
    result = {
        "STIG_ID": "O365-OU-000019",
        "Rule_Title": "Outlook must be configured to not run scripts in forms in which the script and the layout are contained within the message.",
        "Severity": "medium",
        "Status": "Not_Reviewed",
        "Finding_Details": "",
        "Comments": ""
    ...
    try:
        ...
        value_name = "EnableOneOffFormScripts"
        expected_value = 0
        value_type = "DWORD"
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="O365-OU-000019 - Outlook must be configured to not run scripts in forms in which the script and the layout are contained within the message.",
