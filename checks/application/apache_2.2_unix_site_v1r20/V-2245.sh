...
#
# Description:
#     The goal is to completely control the web users experience in navigating any portion of the web document root directories. Ensuring all web content directories have at least the equivalent of an index.html file is a significant factor to accomplish this end. Also, enumeration techniques, such as URL...
#
# Check Content:
#     To view the DocumentRoot value enter the following command: _x000D_ awk '{print \$1,\$2,\$3}' /usr/local/apache2/conf/httpd.conf|grep -i DocumentRoot|grep -v '^#'_x000D_ Note each location following the DocumentRoot string, this is the configured path(s) to the document root directory(s). _x000D_ To view a list of the directories and sub-directories and the file index.html, from each stated DocumentR...
#
# Exit Codes:
