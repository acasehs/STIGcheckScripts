...
#
# Description:
#     CGI scripts represents one of the most common and exploitable means of compromising a web server. By definition, CGI are executable by the operating system of the host server. While access control is provided via the web service, the execution of CGI programs is not otherwise limited unless the SA o...
#
# Check Content:
#     To preclude access to the servers root directory, ensure the following directive is in the httpd.conf file. This entry will also stop users from setting up .htaccess files which can override security features configured in httpd.conf._x000D_ _x000D_ <DIRECTORY /[website root dir]>_x000D_ AllowOverride None_x000D_ </DIRECTORY>_x000D_ _x000D_ If the AllowOverride None is not set, this is a finding._...
#
# Exit Codes:
...
# To preclude access to the servers root directory, ensure the following directive is in the httpd.conf file. This entry will also stop users from setting up .htaccess files which can override security features configured in httpd.conf._x000D_ _x000D_ <DIRECTORY /[website root dir]>_x000D_ AllowOverride None_x000D_ </DIRECTORY>_x000D_ _x000D_ If the AllowOverride None is not set, this is a finding._x000D_
#
# Fix Text from the official STIG:
# Ensure the CGI (or equivalent i.e. scripts) directory has access controls IAW the WEB Services STIG.

echo "TODO: Implement Apache check for V-2228"
