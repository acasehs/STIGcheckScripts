...
# STIG Check: V-221273
# Severity: medium
# Rule Title: OHS must have the mpm_prefork_module directive disabled so as not conflict with the worker directive used to limit the number of allowed simultaneous requests.
# STIG ID: OH12-1X-000002
# Rule ID: SV-221273r960735
#
# Description:
...
echo "TODO: Implement check for V-221273"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OHS must have the mpm_prefork_module directive disabled so as not conflict with the worker directive used to limit the number of allowed simultaneous requests.",
  "status": "$STATUS",
