...
# STIG Check: V-221378
# Severity: medium
# Rule Title: OHS must have the LoadModule authz_user_module directive disabled.
# STIG ID: OH12-1X-000131
# Rule ID: SV-221378r960963
#
# Description:
...
_x000D_
The web server must provide the capability to disable, uninstall, or deactivate functionality and services that are deemed to be non-essential to the web server mission or can adversely impact server performance.  This module provides authorization capabilities so authenticated users can be allowed or denied access to portions
#
# Check Content:
#     If the AO approved system security plan for web server configuration specifies using the OHS authz_user_module in order to meet application architecture requirements,  this requirement can be marked NA._x000D_
_x000D_
1. Open $DOMAIN_HOME/config/fmwconfig/components/OHS/<componentName>/httpd.conf with an editor._x000D_
_x000D_
2. Search for the \"LoadModule authz_user_module\" directive at the OHS server configuration scope._x000D_
_x000D_
