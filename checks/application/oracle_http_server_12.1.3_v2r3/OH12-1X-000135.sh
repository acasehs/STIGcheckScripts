...
# STIG Check: V-221382
# Severity: medium
# Rule Title: OHS must have the LoadModule proxy_http_module directive disabled.
# STIG ID: OH12-1X-000135
# Rule ID: SV-221382r960963
#
# Description:
...
_x000D_
The web server must provide the capability to disable, uninstall, or deactivate functionality and services that are deemed to be non-essential to the web server mission or can adversely impact server performance. The proxy_http_module requires the service of mod_proxy. It provides the features used for proxying HTTP and HTTPS 
#
# Check Content:
#     If the AO approved system security plan for the web server configuration specifies using the proxy_http_module directive in order to meet application architecture requirements and authentication is enforced, this requirement is NA._x000D_
_x000D_
1. Open $DOMAIN_HOME/config/fmwconfig/components/OHS/<componentName>/httpd.conf with an editor._x000D_
_x000D_
2. Search for the \"LoadModule proxy_http_module\" directive at the OHS server configuration scope._x000D_
_x000D_
...
main() {
    ...
    echo "Checking for directive: configuration directive"
    grep -i "configuration directive" "$CONF_FILE" 2>/dev/null || echo "Directive not found"
    echo ""

    echo "MANUAL REVIEW REQUIRED: Verify directive value meets STIG requirements"
    echo "Configuration file: $CONF_FILE"
    echo "Required directive: configuration directive"
