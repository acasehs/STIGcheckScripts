...
Description:
    Specifying where the log files are written gives the system administrator the capability to store the files in a location other than the default, with system files or in a globally accessible location.  The system administrator can also specify a location that is accessible by any enterprise tools t...

Check Content:
    1. As required, open $DOMAIN_HOME/config/fmwconfig/components/OHS/<componentName>/httpd.conf and every .conf file (e.g., ssl.conf) included in it with an editor that contains a "<VirtualHost>" directive.  2. Search for the "RewriteLog" directive at the OHS server and virtual host configuration scope...
...
SEVERITY = "low"
RULE_TITLE = """OHS must have the RewriteLog directive set properly."""
