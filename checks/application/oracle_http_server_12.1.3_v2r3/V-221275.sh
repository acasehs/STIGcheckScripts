...
#
# Check Content:
#     1. Open the $DOMAIN_HOME/config/fmwconfig/components/OHS/<componentName>/httpd.conf file with an editor.  2. Search for the "ThreadsPerChild" directive within "<IfModule mpm_worker_module>" directive at the OHS server configuration scope.  3. If "ThreadsPerChild" is omitted or set greater than "25",...
#
# Exit Codes:
...
echo "TODO: Implement check for V-221275"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "OHS must limit the number of threads within a worker process to limit the number of allowed simultaneous requests.",
  "status": "$STATUS",
