<#
.SYNOPSIS
    DTOO142 - Force encrypted macros to be scanned in open XML documents must be determined and configured._x000D_

.DESCRIPTION
    STIG ID: DTOO142
    Rule Title: Force encrypted macros to be scanned in open XML documents must be determined and configured._x000D_
    Severity: medium
    Vuln ID: V-71073
    Rule ID: SV-85697r1
...
.EXAMPLE
    .\DTOO142.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\DTOO142.ps1 -OutputJson
...
$result = @{
    STIG_ID = "DTOO142"
    Rule_Title = "Force encrypted macros to be scanned in open XML documents must be determined and configured._x000D_"
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
...
try {
    ...
    $regPath = "HKCU:\Software\Policies\Microsoft\Office\16.0\word\security_x000D_"
