<#
.SYNOPSIS
    DTOO123 - Navigation to URLs embedded in Office products must be blocked._x000D_

.DESCRIPTION
    STIG ID: DTOO123
    Rule Title: Navigation to URLs embedded in Office products must be blocked._x000D_
    Severity: medium
    Vuln ID: V-71053
    Rule ID: SV-85677r1
...
.EXAMPLE
    .\DTOO123.ps1 -Config custom_config.json
    Run the check with custom configuration

.EXAMPLE
    .\DTOO123.ps1 -OutputJson
...
$result = @{
    STIG_ID = "DTOO123"
    Rule_Title = "Navigation to URLs embedded in Office products must be blocked._x000D_"
    Severity = "medium"
    Status = "Not_Reviewed"
    Finding_Details = ""
    Comments = ""
