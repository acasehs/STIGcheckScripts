...
#
# Description:
# To make certain that the logging mechanism used by the web server has sufficient storage capacity in which to write the logs, the logging mechanism needs to be able to allocate log record storage capacity.   The task of allocating log record storage capacity is usually performed during initial insta...
#
...
# Configuration
$VulnID = "V-214347"
$StigID = "AS24-W1-000710"
$Severity = "medium"
$RuleTitle = "The Apache web server must use a logging mechanism that is configured to allocate log record storage capacity large enough to accommodate the logging requirements of the Apache web server."

# Show help
if ($Help) {
    Write-Host "Usage: .\V-214347.ps1 [-ConfigFile FILE] [-OutputJson FILE] [-Help]"
    Write-Host ""
    Write-Host "Options:"
