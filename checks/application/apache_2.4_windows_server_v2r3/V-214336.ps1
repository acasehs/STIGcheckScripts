...
#
# Description:
# Determining a safe state for failure and weighing that against a potential DoS for users depends on what type of application the web server is hosting. For an application presenting publicly available information that is not critical, a safe state for failure might be to shut down for any type of fa...
#
...
# Configuration
$VulnID = "V-214336"
$StigID = "AS24-W1-000550"
$Severity = "medium"
$RuleTitle = "The Apache web server must be built to fail to a known safe state if system initialization fails, shutdown fails, or aborts fail."

# Show help
if ($Help) {
    Write-Host "Usage: .\V-214336.ps1 [-ConfigFile FILE] [-OutputJson FILE] [-Help]"
    Write-Host ""
    Write-Host "Options:"
