...
STIG ID: AS24-W1-000240
Rule ID: SV-214318r960963

Description:
    User management and authentication can be an essential part of any application hosted by the web server. Along with authenticating users, the user management function must perform several other tasks such as password complexity, locking users after a configurable number of failed logons, and managem...

Check Content:
    Interview the System Administrator (SA) about the role of the Apache web server.  If the web server is hosting an application, have the SA provide supporting documentation on how the application's user management is accomplished outside of the web server.  If the web server is not hosting an application, this is Not Applicable.  If the web server is performing user management for hosted applicatio...
...
# Check metadata
VULN_ID = "V-214318"
STIG_ID = "AS24-W1-000240"
SEVERITY = "medium"
RULE_TITLE = """The Apache web server must not perform user management for hosted applications."""
