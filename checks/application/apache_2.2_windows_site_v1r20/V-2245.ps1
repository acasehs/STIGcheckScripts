...
#
# Description:
# The goal is to completely control the web users experience in navigating any portion of the web document root directories. Ensuring all web content directories have indexing turned off or at least the equivalent of an index.html file is a significant factor to accomplish this end. Enumeration techni...
#
...
$StigID = "WG170 W22"
$Severity = "low"
$RuleTitle = "Each readable web document directory must contain either a default, home, index, or equivalent file."

# Show help
if ($Help) {
    Write-Host "Usage: .\V-2245.ps1 [-ConfigFile FILE] [-OutputJson FILE] [-Help]"
    Write-Host ""
    Write-Host "Options:"
...
function Invoke-StigCheck {
    ...
    # Locate the Apache httpd.conf file._x000D_ _x000D_ If unable to locate the file, perform a search of the system to find the location of the file._x000D_ _x000D_ Open the httpd.conf file with an editor such as notepad, and search for the following uncommented directive: DocumentRoot_x000D_ _x000D_ Note the name of the DocumentRoot directory._x000D_ _x000D_ Review the results for each document root directory and its subdirectories. If a directory does not contain an index.html or equivalent default...
    #
    # Fix Text:
