...
#
# Description:
# Resource exhaustion can occur when an unlimited number of concurrent requests are allowed on a web site, facilitating a denial of service attack. Mitigating this kind of attack will include limiting the number of concurrent HTTP/HTTPS requests per IP address and may include, where feasible, limiting...
#
...
$StigID = "WG110 W22"
$Severity = "medium"
$RuleTitle = "The number of allowed simultaneous requests must be set."

# Show help
if ($Help) {
    Write-Host "Usage: .\V-2240.ps1 [-ConfigFile FILE] [-OutputJson FILE] [-Help]"
    Write-Host ""
    Write-Host "Options:"
...
function Invoke-StigCheck {
    ...
    # NOTE: This setting must be explicitly set._x000D_ _x000D_ Locate the Apache httpd.conf file._x000D_ _x000D_ If unable to locate the file, perform a search of the system to find the location of the file._x000D_ _x000D_ Open the httpd.conf file with an editor such as notepad, and search for the following uncommented directive: MaxKeepAliveRequests_x000D_ _x000D_ Every enabled MaxKeepAliveRequests value needs to be 100 or greater. If any directive is less than 100, this is a finding.
    #
    # Fix Text:
