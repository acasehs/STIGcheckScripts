...
Description:
    This check verifies that DoD is a hosted web site's CA. The certificate is actually a DoD-issued server certificate used by the organization being reviewed. This is used to verify the authenticity of the web site to the user. If the certificate is not for the server (Certificate belongs to), if the ...

Check Content:
    Open browser window and browse to the appropriate site. Before entry to the site, you should be presented with the server's DoD PKI credentials. Review these credentials for authenticity._x000D_ _x000D_ Find an entry which cites:_x000D_ _x000D_ Issuer:_x000D_ CN = DOD CLASS 3 CA-3_x000D_ OU = PKI_x000D_ OU = DoD_x000D_ O = U.S. Government_x000D_ C = US_x000D_ _x000D_ If the server is running as a ...
...
STIG_ID = "WG350 W22"
SEVERITY = "medium"
RULE_TITLE = """A private web server must have a valid DoD server certificate."""
...
def run_check(config=None):
    ...
    # Open browser window and browse to the appropriate site. Before entry to the site, you should be presented with the server's DoD PKI credentials. Review these credentials for authenticity._x000D_ _x000D_ Find an entry which cites:_x000D_ _x000D_ Issuer:_x000D_ CN = DOD CLASS 3 CA-3_x000D_ OU = PKI_x000D_ OU = DoD_x000D_ O = U.S. Government_x000D_ C = US_x000D_ _x000D_ If the server is running as a public web server, this finding should be Not Applicable._x000D_ _x000D_ NOTE: In some cases, the we...
    #
