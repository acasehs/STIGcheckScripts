...
$StigID = "WG242 W22"
$Severity = "medium"
$RuleTitle = "Log file data must contain required data elements."

# Show help
if ($Help) {
    Write-Host "Usage: .\V-13688.ps1 [-ConfigFile FILE] [-OutputJson FILE] [-Help]"
    Write-Host ""
    Write-Host "Options:"
...
function Invoke-StigCheck {
    ...
    # To verify the log settings:_x000D_ _x000D_ Default Windows location: :\Program Files\Apache Group\Apache2\logs\access.log or :\Program Files\Apache Software Foundation\Apache2.2\logs\access.log._x000D_ _x000D_ If these directories do not exist, you can search the web server for the httpd.conf config file to determine the location of the logs._x000D_ _x000D_ Items to be logged are as shown in this sample line in the httpd.conf file:_x000D_ _x000D_ LogFormat `"%a %A %h %H %l %m %s %t %u %U \`"%{Refe...
    #
    # Fix Text:
