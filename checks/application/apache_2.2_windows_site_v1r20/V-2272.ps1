...
$StigID = "WG460 W22"
$Severity = "medium"
$RuleTitle = "PERL scripts must use the TAINT option."

# Show help
if ($Help) {
    Write-Host "Usage: .\V-2272.ps1 [-ConfigFile FILE] [-OutputJson FILE] [-Help]"
    Write-Host ""
    Write-Host "Options:"
...
function Invoke-StigCheck {
    ...
    # Locate the Apache httpd.conf file._x000D_ _x000D_ If unable to locate the file, perform a search of the system to find the location of the file._x000D_ _x000D_ Open the httpd.conf file with an editor such as Notepad, and search for the following uncommented directive: ScriptInterpreterSource_x000D_ _x000D_ For any enabled ScriptInterpreterSource directives the only authorized entries are Registry-Strict or Script. If any other entry (i.e. Registry) is found, this is a finding._x000D_ _x000D_ For...
    #
    # Fix Text:
