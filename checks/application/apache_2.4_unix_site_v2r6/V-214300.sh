...
#
# Description:
#     Non-DOD approved PKIs have not been evaluated to ensure that they have security controls and identity vetting procedures in place that are sufficient for DOD systems to rely on the identity asserted in the certificate. PKIs lacking sufficient security controls and identity vetting procedures risk be...
#
# Check Content:
#     Verify the â€œssl moduleâ€ module is loaded: # httpd -M | grep -i ssl_module Output:  ssl_module (shared)   If the \"ssl_module\" is not found, this is a finding.   Determine the location of the ssl.conf file: # find / -name ssl.conf Output: /etc/httpd/conf.d/ssl.conf  Search the ssl.conf file for the following: # cat /etc/httpd/conf.d/ssl.conf | grep -i \"SSLCACertificateFile\" Output should be simi...
#
# Exit Codes:
...
# Verify the â€œssl moduleâ€ module is loaded: # httpd -M | grep -i ssl_module Output:  ssl_module (shared)   If the \"ssl_module\" is not found, this is a finding.   Determine the location of the ssl.conf file: # find / -name ssl.conf Output: /etc/httpd/conf.d/ssl.conf  Search the ssl.conf file for the following: # cat /etc/httpd/conf.d/ssl.conf | grep -i \"SSLCACertificateFile\" Output should be similar to: SSLCACertificateFile /etc/pki/tls/certs/ca-bundle.crt  Review the path of the \"SSLCACertific...
#
...
echo "TODO: Implement Apache check for V-214300"
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "The Apache web server must only accept client certificates issued by DOD PKI or DoD-approved PKI Certification Authorities (CAs).",
  "status": "$STATUS",
