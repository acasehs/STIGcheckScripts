...
Rule ID: SV-214378r1043181

Description:
    Communication between a client and the web server is done using the HTTP protocol, but HTTP is a stateless protocol. To maintain a connection or session, a web server will generate a session identifier (ID) for each client session when the session is initiated. The session ID allows the web server t...
...
# Check metadata
VULN_ID = "V-214378"
STIG_ID = "AS24-W2-000500"
SEVERITY = "medium"
RULE_TITLE = """The Apache web server must generate unique session identifiers that cannot be reliably reproduced."""
