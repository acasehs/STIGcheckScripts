...
# STIG Check: V-234193
# Severity: high
# Rule Title: The FortiGate device must be running an operating system release that is currently supported by the vendor.
# STIG ID: FGFW-ND-000170
# Rule ID: SV-234193r879887
#
# Description:
...
#
# Check Content:
#     Log in to the Fortinet Support Portal and review the Product Life Cycle Software "End of Support Date". _x000D_
#     _x000D_
#     Log in to the FortiGate with Super-Admin privilege in the GUI and review the Dashboard >> Status >> System Information widget for Firmware version. _x000D_
#     _x000D_
...
main() {
    ...
    #     echo "PASS: Check FGFW-ND-000170 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check FGFW-ND-000170 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for FGFW-ND-000170"
    echo "Description: Network devices running an unsupported operating system lack current security fixes required to mitigate the risks associated with recent vulnerabilities."
