...
STIG ID: CNTR-K8-000960
Rule ID: SV-242414r1043177
...
Note: In the above command, "podname" is the name of the pod. For the command to work correctly, the current context must be changed to the namespace for the pod. The comma
...
def perform_check(config):
    ...
    # Check Content: On the Control Plane, run the command:
kubectl get pods --all-namespaces

The list returned is all pods running within the Kubernetes cluster. For those pods running within the user namespaces (System...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - The Kubernetes cluster must use non-privileged host ports for user pods.",
