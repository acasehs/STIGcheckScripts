...
# STIG ID: CNTR-K8-000700
# Rule ID: SV-242403r986135
#
# Description:
...
#     
#     The API server policy file allows for the following levels of audit
#
# Check Content:
...
#     apiVersion: audit.k8s.io/vX (Where X is the latest api
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check CNTR-K8-000700 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check CNTR-K8-000700 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for CNTR-K8-000700"
    echo "Description: Within Kubernetes, audit data for all components is generated by the API server. This audit data is important when there are issues, to include security incidents that must be investigated. To make the audit data worthwhile for the investigation of events, it is necessary to have the appropriate and required data logged. To fully understand the event, it is important to identify any users associated with the event. 
#     
#     The API server policy file allows for the following levels of audit"
