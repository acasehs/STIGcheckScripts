...
# STIG ID: CNTR-K8-003160
# Rule ID: SV-242449r961863
#
# Description:
#     The Kubernetes kubelet certificate authority file contains settings for the Kubernetes Node TLS certificate authority. Any request presenting a client certificate signed by one of the authorities in the client-ca-file is authenticated with an identity corresponding to the CommonName of the client certificate. If this file can be changed, the Kubernetes architecture could be compromised. The scheduler will implement the changes immediately. Many of the security settings within the document are im
#
# Check Content:
#     On the Control Plane, run the command:_x000D_
#     ps -ef | grep kubelet_x000D_
#     _x000D_
#     If the "--client-ca-file" option exists, this is a finding. _x000D_
#     _x000D_
#     Note the path to the config file (identified by --config)._x000D_
#     _x000D_
#     Run the command:_x000D_
#     grep -i clientCAFile <path_to_config_file>_x000D_
#     _x000D_
#     Note the path to the client ca file._x000D_
#     _x000D_
#     Run the command:_x000D_
#     stat -c %a <path_to_client_ca_f
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check CNTR-K8-003160 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check CNTR-K8-003160 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for CNTR-K8-003160"
    echo "Description: The Kubernetes kubelet certificate authority file contains settings for the Kubernetes Node TLS certificate authority. Any request presenting a client certificate signed by one of the authorities in the client-ca-file is authenticated with an identity corresponding to the CommonName of the client certificate. If this file can be changed, the Kubernetes architecture could be compromised. The scheduler will implement the changes immediately. Many of the security settings within the document are im"
