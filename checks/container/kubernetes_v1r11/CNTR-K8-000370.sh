...
#     
#     While there are instances where anonymous connections may be needed (e.g., health checks
#
# Check Content:
...
main() {
    ...
    #     echo "PASS: Check CNTR-K8-000370 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check CNTR-K8-000370 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for CNTR-K8-000370"
    echo "Description: A user who has access to the Kubelet essentially has root access to the nodes contained within the Kubernetes Control Plane. To control access, users must be authenticated and authorized. By allowing anonymous connections, the controls put in place to secure the Kubelet can be bypassed.
#     
#     Setting anonymous authentication to "false" also disables unauthenticated requests from kubelets.
#     
#     While there are instances where anonymous connections may be needed (e.g., health checks"
