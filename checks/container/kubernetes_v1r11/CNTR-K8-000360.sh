...
# STIG ID: CNTR-K8-000360
# Rule ID: SV-242390r960792
#
# Description:
...
#     
#     While there are instances where anonymous connections may be
#
# Check Content:
...
main() {
    ...
    #     echo "PASS: Check CNTR-K8-000360 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check CNTR-K8-000360 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for CNTR-K8-000360"
    echo "Description: The Kubernetes API Server controls Kubernetes via an API interface. A user who has access to the API essentially has root access to the entire Kubernetes cluster. To control access, users must be authenticated and authorized. By allowing anonymous connections, the controls put in place to secure the API can be bypassed.
#     
#     Setting "--anonymous-auth" to "false" also disables unauthenticated requests from kubelets.
#     
#     While there are instances where anonymous connections may be"
