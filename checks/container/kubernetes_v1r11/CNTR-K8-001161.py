...
STIG Check: V-274883
Severity: high
Rule Title: Sensitive information must be stored using Kubernetes Secrets or an external Secret store provider.
STIG ID: CNTR-K8-001161
Rule ID: SV-274883r1107230
...
def perform_check(config):
    ...
    # Check Content: On the Kubernetes Master node, run the following command:
kubectl get all,cm -A -o yaml 

Manually review the output for sensitive information.

If any sensitive information is found, this is a findin...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - Sensitive information must be stored using Kubernetes Secrets or an external Secret store provider.",
