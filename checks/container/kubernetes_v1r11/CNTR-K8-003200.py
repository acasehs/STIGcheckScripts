...
STIG Check: V-242453
Severity: medium
Rule Title: The Kubernetes kubelet KubeConfig file must be owned by root.
STIG ID: CNTR-K8-003200
Rule ID: SV-242453r961863
...
def perform_check(config):
    ...
    # Check Content: Review the Kubernetes Kubelet conf files by using the command:

stat -c %U:%G /etc/kubernetes/kubelet.conf| grep -v root:root

If the command returns any non root:root file permissions, this is a find...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - The Kubernetes kubelet KubeConfig file must be owned by root.",
