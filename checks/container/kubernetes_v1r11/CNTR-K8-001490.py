...
STIG ID: CNTR-K8-001490
Rule ID: SV-242427r1043178
...
The communication session is protected by utilizing 
...
def perform_check(config):
    ...
    # Check Content: Change to the /etc/kubernetes/manifests directory on the Kubernetes Control Plane. 

Run the command:
    grep -i key-file *

If the setting "key-file" is not configured in the etcd manifest  file, th...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - Kubernetes etcd must have a key file for secure communication.",
