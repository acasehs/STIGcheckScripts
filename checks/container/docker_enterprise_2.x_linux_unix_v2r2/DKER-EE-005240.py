...
By default, the permissions for registry certificate files might not be 444. The default file permissions are governed by the system or user specific uma
...
def perform_check(config):
    ...
    # Check Content: Ensure that registry certificate file permissions are set to 444 or more restrictive.

Execute the below command to verify that the registry certificate files have permissions of 444 or more restricti...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - Docker Enterprise registry certificate file permissions must be set to 444 or more restrictive.",
