...
# STIG Check: V-235809
# Severity: high
# Rule Title: Docker Enterprise host devices must not be directly exposed to containers.
# STIG ID: DKER-EE-002040
# Rule ID: SV-235809r960963
#
# Description:
...
#     
#     docker ps --quiet --all | xargs docker inspect --format '{{ .Id }}: D
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-002040 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-002040 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-002040"
    echo "Description: Host devices can be directly exposed to containers at runtime. Do not directly expose host devices to containers especially for containers that are not trusted.
#     
#     The --device option exposes the host devices to the containers and consequently, the containers can directly access such host devices. Do not require the container to run in privileged mode to access and manipulate the host devices. By default, the container will be able to read, write and mknod these devices. Additionally, "
