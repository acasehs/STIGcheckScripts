...
#     
#     (Docker Engine - Enterprise: Standalone) - The majority of
#
# Check Content:
...
#     via CLI:
#     
#     Linux: Execute the following command and verify the certificate chain in the output is valid and matches that of the trusted
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-001870 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-001870 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-001870"
    echo "Description: Docker Enterprise includes the following capabilities that are considered non-essential:
#     
#     *NOTE: disabling these capabilities negatively affects the operation of Docker Trusted Registry (DTR) and should be disregarded when UCP and DTR are installed. The security capabilities provided by UCP and DTR offset any potential vulnerabilities associated with not disabling these essential capabilities the Engine provides.
#     
#     (Docker Engine - Enterprise: Standalone) - The majority of"
