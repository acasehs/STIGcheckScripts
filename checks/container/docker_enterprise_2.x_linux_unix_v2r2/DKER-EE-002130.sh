...
# STIG Check: V-235818
# Severity: high
# Rule Title: The Docker Enterprise socket must not be mounted inside any containers.
# STIG ID: DKER-EE-002130
# Rule ID: SV-235818r960963
#
# Description:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-002130 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-002130 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-002130"
    echo "Description: The docker socket docker.sock (Linux) and \\.\pipe\docker_engine (Windows) should not be mounted inside a container, with the exception case being during the installation of Universal Control Plane (UCP) component of Docker Enterprise as it is required for install.
#     
#     If the docker socket is mounted inside a container it would allow processes running within the container to execute docker commands which effectively allows for full control of the host.
#     
#     By default, docker.so"
