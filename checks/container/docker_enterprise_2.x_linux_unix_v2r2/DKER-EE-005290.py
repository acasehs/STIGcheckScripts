...
By default, the ownership and group-ownership for Docker server certificate key file is corr
...
def perform_check(config):
    ...
    # Check Content: Ensure that Docker server certificate key file ownership is set to root:root.
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - Docker Enterprise server certificate key file ownership must be set to root:root.",
