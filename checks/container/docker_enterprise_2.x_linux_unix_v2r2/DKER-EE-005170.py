...
If the above command returns nothing, this is not a finding. If the command re
...
def perform_check(config):
    ...
    # Check Content: Ensure that docker.service file ownership is set to root:root

Step 1: Find out the file location:

systemctl show -p FragmentPath docker.service

Step 2: If the file does not exist, this is not a fin...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - Docker Enterprise docker.service file ownership must be set to root:root.",
