...
STIG Check: V-235794
Severity: medium
Rule Title: The Docker Enterprise self-signed certificates in Docker Trusted Registry (DTR) must be replaced with DoD trusted, signed certificates.
STIG ID: DKER-EE-001880
Rule ID: SV-235794r960963
...
(Docker Engine - Enterprise: Standalone) - The majority of these items wer
...
echo "" | openssl s_client -connect [dtr_u
...
def perform_check(config):
    ...
    # Check Content: Check that DTR has been integrated with a trusted certificate authority (CA).

via UI:

In the DTR web console, navigate to "System" | "General" and click on the "Show TLS settings" link in the "Domai...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - The Docker Enterprise self-signed certificates in Docker Trusted Registry (DTR) must be replaced with DoD trusted, signed certificates.",
