...
PID namespace provides separation of processes. The PID Namespace removes the view of the system processes, and allows process IDs to be reused including PID 1. If the host's PID namespace is shared with the container, it would allow processes within the container to see all of the processes on the ho
...
def perform_check(config):
    ...
    # Check Content: This check only applies to the use of Docker Engine - Enterprise on a Linux host operating system.

Ensure the host's process namespace is not shared.

via CLI:

Linux: As a Docker EE Admin, execute t...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - The Docker Enterprise hosts process namespace must not be shared.",
