...
STIG Check: V-235836
Severity: medium
Rule Title: The Docker Enterprise log aggregation/SIEM systems must be configured to send an alert the ISSO/ISSM when unauthorized software is installed.
STIG ID: DKER-EE-003460
Rule ID: SV-235836r961452

Description:
    A Docker image is analogous to software in the context of this control.
...
The UCP and DTR components of Docker Enterprise prov
...
def perform_check(config):
    ...
    # STIG ID: DKER-EE-003460
    # Check Type: unknown
    # Check Content: Work with the SIEM administrator to determine if an alert is configured to notify the ISSO/ISSM when unauthorized software is installed on Docker nodes.

If there is no alert configured, this is a fin...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - The Docker Enterprise log aggregation/SIEM systems must be configured to send an alert the ISSO/ISSM when unauthorized software is installed.",
