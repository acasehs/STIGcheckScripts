...
# STIG Check: V-235840
# Severity: medium
# Rule Title: Vulnerability scanning must be enabled for all repositories in the Docker Trusted Registry (DTR) component of Docker Enterprise.
# STIG ID: DKER-EE-003840
# Rule ID: SV-235840r961563
#
# Description:
...
#     
#     If "Enable Scanning" is tuned off or if the vulnerability database is not
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-003840 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-003840 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-003840"
    echo "Description: DTR can scan Docker images for vulnerabilities and this capability should be enabled to meet the requirements of this control.
#     
#     When enabled, for every Docker image that is pushed to DTR, a scan of each of the image layers is conducted. An analysis of all packages and compiled binaries is done for each image layer and if a package or binary is associated with a known vulnerability as identified by the MITRE CVE or NIST NVD databases, then it is flagged in DTR."
