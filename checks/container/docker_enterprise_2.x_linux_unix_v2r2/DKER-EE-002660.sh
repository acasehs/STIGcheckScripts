...
# STIG Check: V-235826
# Severity: medium
# Rule Title: Docker Secrets must be used to store configuration files and small amounts of user-generated data (up to 500 kb in size) in Docker Enterprise.
# STIG ID: DKER-EE-002660
# Rule ID: SV-235826r961128
#
# Description:
...
#     docker secret ls
#     
#     Confirm containerized applications identified in SSP as utilizing Docker secrets have a corresponding secret configu
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-002660 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-002660 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-002660"
    echo "Description: By leveraging Docker Secrets or Kubernetes secrets to store configuration files and small amounts of user-generated data (up to 500 kb in size), the data is encrypted at rest by the Engine's FIPS-validated cryptography."
