...
# STIG Check: V-235864
# Severity: high
# Rule Title: Docker Enterprise server certificate key file permissions must be set to 400.
# STIG ID: DKER-EE-005300
# Rule ID: SV-235864r961863
#
# Description:
...
#     
#     By default, the permissions for Docker server certificate key file might not be 400. Th
#
# Check Content:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-005300 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-005300 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-005300"
    echo "Description: Verify that the Docker server certificate key file (the file that is passed along with --TLSkey parameter) has permissions of 400.
#     
#     The Docker server certificate key file should be protected from any tampering or unneeded reads. It holds the private key for the Docker server certificate. Hence, it must have permissions of 400 to maintain the integrity of the Docker server certificate.
#     
#     By default, the permissions for Docker server certificate key file might not be 400. Th"
