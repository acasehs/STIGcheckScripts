...
#     
#     Ensure that PidsLimit is not set to 0 or -1. A PidsLimit of 0 or -1 means 
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-002780 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-002780 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-002780"
    echo "Description: Use --pids-limit flag at container runtime.
#     
#     Attackers could launch a fork bomb with a single command inside the container. This fork bomb can crash the entire system and requires a restart of the host to make the system functional again. PIDs cgroup --pids-limit will prevent this kind of attacks by restricting the number of forks that can happen inside a container at a given time.
#     
#     The Default value for --pids-limit is 0 which means there is no restriction on the number "
