...
STIG Check: V-235793
Severity: medium
Rule Title: The Docker Enterprise self-signed certificates in Universal Control Plane (UCP) must be replaced with DoD trusted, signed certificates.
STIG ID: DKER-EE-001870
Rule ID: SV-235793r960963
...
(Docker Engine - Enterprise: Standalone) - The majority of these items were origin

Check Content:
    Check that UCP has been integrated with a trusted certificate authority (CA).
...
echo "" | openssl s_client -connect [ucp_u
...
def perform_check(config):
    ...
    # Check Content: Check that UCP has been integrated with a trusted certificate authority (CA).

via UI:

In the UCP web console, navigate to "Admin Settings" | "Certificates" and click on the "Download UCP Server CA C...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - The Docker Enterprise self-signed certificates in Universal Control Plane (UCP) must be replaced with DoD trusted, signed certificates.",
