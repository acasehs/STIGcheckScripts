...
# STIG Check: V-235804
# Severity: medium
# Rule Title: Only required ports must be open on the containers in Docker Enterprise.
# STIG ID: DKER-EE-001990
# Rule ID: SV-235804r960963
#
# Description:
...
#     
#     A container can be run just with the ports defined in the Dockerfile for its image or can be arbitrarily passed run time parameters to open a list of ports. Additionally, over time, Dockerfile may undergo various changes and the list of exposed ports may or may not be relevant to the applicat
#
# Check Content:
...
#     
#     Review the list and ensure that the ports mapped are the ones that a
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-001990 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-001990 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-001990"
    echo "Description: Dockerfile for a container image defines the ports to be opened by default on a container instance. The list of ports may or may not be relevant to the application running within the container.
#     
#     A container can be run just with the ports defined in the Dockerfile for its image or can be arbitrarily passed run time parameters to open a list of ports. Additionally, over time, Dockerfile may undergo various changes and the list of exposed ports may or may not be relevant to the applicat"
