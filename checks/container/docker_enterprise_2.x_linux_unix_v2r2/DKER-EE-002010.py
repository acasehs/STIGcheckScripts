...
By default, container can use all of the memory on the host. The user can use memory limit mechanism to prevent a denial of service arising from one container consuming all of the hostâ€™s resources such that other containers on the same host cannot perform their intended 
...
def perform_check(config):
    ...
    # Check Content: Ensure memory limits are in place for all containers.

This check should be executed on all nodes in a Docker Enterprise cluster.

via CLI:

Linux: As a Docker EE Admin, execute the following command ...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - Memory usage for all containers must be limited in Docker Enterprise.",
