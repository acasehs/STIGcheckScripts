...
# STIG Check: V-235827
# Severity: medium
# Rule Title: Docker Enterprise container health must be checked at runtime.
# STIG ID: DKER-EE-002770
# Rule ID: SV-235827r961155
#
# Description:
...
#     
#     By default, health checks are not done at container
#
# Check Content:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-002770 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-002770 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-002770"
    echo "Description: If the container image does not have an HEALTHCHECK instruction defined, use --health-cmd parameter at container runtime for checking container health.
#     
#     One of the important security triads is availability. If the container image being used does not have a pre-defined HEALTHCHECK instruction, use the --health-cmd parameter to check container health at runtime. Based on the reported health status, take necessary actions.
#     
#     By default, health checks are not done at container"
