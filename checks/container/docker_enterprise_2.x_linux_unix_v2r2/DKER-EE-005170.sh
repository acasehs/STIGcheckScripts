...
# STIG Check: V-235851
# Severity: high
# Rule Title: Docker Enterprise docker.service file ownership must be set to root:root.
# STIG ID: DKER-EE-005170
# Rule ID: SV-235851r961863
#
# Description:
...
#     
#     This file may not be present on the system. In that case, this recommendation is not applicable. By default, if the file is present, the ownership and group-ownership for this file is correctly se
#
# Check Content:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-005170 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-005170 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-005170"
    echo "Description: Verify that the docker.service file ownership and group-ownership are correctly set to root.
#     
#     docker.service file contains sensitive parameters that may alter the behavior of Docker daemon. Hence, it should be owned and group-owned by root to maintain the integrity of the file.
#     
#     This file may not be present on the system. In that case, this recommendation is not applicable. By default, if the file is present, the ownership and group-ownership for this file is correctly se"
