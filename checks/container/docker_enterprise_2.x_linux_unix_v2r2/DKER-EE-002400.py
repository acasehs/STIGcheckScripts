...
STIG Check: V-235823
Severity: medium
Rule Title: Docker Enterprise Swarm manager must be run in auto-lock mode.
STIG ID: DKER-EE-002400
Rule ID: SV-235823r961041
...
With --autolock enabled, when Docker restarts, unlock the swarm first, using a key encryp
...
def perform_check(config):
    ...
    # Check Content: Ensure swarm manager is run in auto-lock mode.

via CLI:

Linux: As a Docker EE Admin, follow the steps below using a Universal Control Plane (UCP) client bundle:

Run the below command. If it outputs...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - Docker Enterprise Swarm manager must be run in auto-lock mode.",
