...
STIG Check: V-235833
Severity: medium
Rule Title: All Docker Engine - Enterprise nodes must be configured with a log driver plugin that sends logs to a remote log aggregation system (SIEM).
STIG ID: DKER-EE-003320
Rule ID: SV-235833r961395
...
Verify that the "log-driver" property is set to one of the following: "sys...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - All Docker Engine - Enterprise nodes must be configured with a log driver plugin that sends logs to a remote log aggregation system (SIEM).",
