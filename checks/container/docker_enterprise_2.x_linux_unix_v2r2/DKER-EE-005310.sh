...
# STIG Check: V-235865
# Severity: high
# Rule Title: Docker Enterprise socket file ownership must be set to root:docker.
# STIG ID: DKER-EE-005310
# Rule ID: SV-235865r961863
#
# Description:
...
#     
#     Additionally, the Docker installer creates a UNI
#
# Check Content:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-005310 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-005310 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-005310"
    echo "Description: Verify that the Docker socket file is owned by root and group-owned by docker.
#     
#     Docker daemon runs as root. The default UNIX socket hence must be owned by root. If any other user or process owns this socket, then it might be possible for that non-privileged user or process to interact with Docker daemon. Also, such a non-privileged user or process might interact with containers. This is neither secure nor desired behavior.
#     
#     Additionally, the Docker installer creates a UNI"
