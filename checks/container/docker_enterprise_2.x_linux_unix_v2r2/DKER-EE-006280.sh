...
# STIG Check: V-235874
# Severity: medium
# Rule Title: Docker Enterprise Universal Control Plane (UCP) must be configured to use TLS 1.2.
# STIG ID: DKER-EE-006280
# Rule ID: SV-235874r961869
#
# Description:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-006280 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-006280 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-006280"
    echo "Description: By default docker UCP is configured to use TLS v1.2, if this setting is misconfigured, older protocols containing security weaknesses could be utilized. TLS requires a handshake between client and server which is where the TLS version utilized in the connection is negotiated. For DoD use cases, all TLS must be at version 1.2."
