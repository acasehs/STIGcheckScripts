...
# STIG Check: V-235807
# Severity: low
# Rule Title: Docker Enterprise CPU priority must be set appropriately on all containers.
# STIG ID: DKER-EE-002020
# Rule ID: SV-235807r960963
#
# Description:
...
#     
#     By default, CPU time is divided between containers equally. If it is desired, to control the CPU time amongst the container instances, use CPU sharing feature. CPU sharing allows to prioritize one container over the other and forbids the lower priority container
#
# Check Content:
...
#     
#     If the above command returns 0 or 1024, it means the CPU shares are not in place 
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-002020 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-002020 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-002020"
    echo "Description: By default, all containers on a Docker host share the resources equally. By using the resource management capabilities of Docker host, such as CPU shares, the user control the host CPU resources that a container may consume.
#     
#     By default, CPU time is divided between containers equally. If it is desired, to control the CPU time amongst the container instances, use CPU sharing feature. CPU sharing allows to prioritize one container over the other and forbids the lower priority container"
