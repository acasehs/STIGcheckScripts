...
STIG Check: V-235786
Severity: medium
Rule Title: log-opts on all Docker Engine - Enterprise nodes must be configured.
STIG ID: DKER-EE-001370
Rule ID: SV-235786r960882
...
Verify that the "log-opts" object includes the "max-size" and "max-file" properties and that they are set according
...
def perform_check(config):
    ...
    # Check Content: Verify this check on all Docker Engine - Enterprise nodes in the cluster.

via CLI:

Linux: Execute the following commands as a trusted user on the host operating system:
Note: daemon.json file does n...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - log-opts on all Docker Engine - Enterprise nodes must be configured.",
