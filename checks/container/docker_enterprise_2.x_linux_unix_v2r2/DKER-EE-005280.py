...
STIG Check: V-235862
Severity: medium
Rule Title: Docker Enterprise server certificate file permissions must be set to 444 or more restrictive.
STIG ID: DKER-EE-005280
Rule ID: SV-235862r961863
...
By default, the permissions for Docker server certificate file might not be 444. The default file permissions 
...
def perform_check(config):
    ...
    # Check Content: Ensure that Docker server certificate file permissions are set to 444 or more restrictive.

Execute the below command to verify that the Docker server certificate file has permissions of 444 or more r...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - Docker Enterprise server certificate file permissions must be set to 444 or more restrictive.",
