...
STIG Check: V-235850
Severity: medium
Rule Title: Docker Enterprise node certificates must be rotated as defined in the System Security Plan (SSP).
STIG ID: DKER-EE-005080
Rule ID: SV-235850r961863
...
def perform_check(config):
    ...
    # Check Content: Ensure node certificates are rotated as appropriate.

via CLI:

Linux: As a Docker EE Admin, follow the steps below using a Universal Control Plane (UCP) client bundle:

Run the below command and ensu...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - Docker Enterprise node certificates must be rotated as defined in the System Security Plan (SSP).",
