...
STIG Check: V-235834
Severity: medium
Rule Title: Log aggregation/SIEM systems must be configured to alarm when audit storage space for Docker Engine - Enterprise nodes exceed 75% usage.
STIG ID: DKER-EE-003330
Rule ID: SV-235834r961398
...
def perform_check(config):
    ...
    # STIG ID: DKER-EE-003330
    # Check Type: unknown
    # Check Content: Work with the SIEM administrator to determine if an alert is configured to alarm when audit storage space for Docker Engine - Enterprise nodes exceed 75% usage.

If there is no alert configured, this ...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - Log aggregation/SIEM systems must be configured to alarm when audit storage space for Docker Engine - Enterprise nodes exceed 75% usage.",
