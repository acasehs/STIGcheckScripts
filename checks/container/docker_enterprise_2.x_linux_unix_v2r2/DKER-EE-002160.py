...
STIG Check: V-235820
Severity: medium
Rule Title: Docker Enterprise incoming container traffic must be bound to a specific host interface.
STIG ID: DKER-EE-002160
Rule ID: SV-235820r960966
...
If there are multiple network interfaces on the host machine, the container can accept connections on the exposed ports on any network interface. This might not be desire
...
Review the list and ensure that the exposed container por
...
def perform_check(config):
    ...
    # Check Content: Ensure incoming container traffic is bound to a specific host interface.

This check should be executed on all nodes in a Docker Enterprise cluster.

via CLI:

Linux: As a Docker EE Admin, execute the...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - Docker Enterprise incoming container traffic must be bound to a specific host interface.",
