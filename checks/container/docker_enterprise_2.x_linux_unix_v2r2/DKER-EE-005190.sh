...
# STIG Check: V-235853
# Severity: high
# Rule Title: Docker Enterprise docker.socket file ownership must be set to root:root.
# STIG ID: DKER-EE-005190
# Rule ID: SV-235853r961863
#
# Description:
...
#     
#     This file may not be present on the system. In that case, this recommendation is not applicable. By default, if the file is present, the ownership and group-ownership for this file is correctly s
#
# Check Content:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-005190 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-005190 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-005190"
    echo "Description: Verify that the docker.socket file ownership and group ownership is correctly set to root.
#     
#     docker.socket file contains sensitive parameters that may alter the behavior of Docker remote API. Hence, it should be owned and group-owned by root to maintain the integrity of the file.
#     
#     This file may not be present on the system. In that case, this recommendation is not applicable. By default, if the file is present, the ownership and group-ownership for this file is correctly s"
