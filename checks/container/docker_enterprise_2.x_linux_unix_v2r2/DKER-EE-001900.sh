...
# STIG Check: V-235796
# Severity: medium
# Rule Title: The Create repository on push option in Docker Trusted Registry (DTR) must be disabled in Docker Enterprise.
# STIG ID: DKER-EE-001900
# Rule ID: SV-235796r960963
#
# Description:
...
#     
#     AUTHTOKEN=$(curl -sk -u <username>:<password> "https://[dtr_url]/auth/token" | jq -r 
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-001900 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-001900 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-001900"
