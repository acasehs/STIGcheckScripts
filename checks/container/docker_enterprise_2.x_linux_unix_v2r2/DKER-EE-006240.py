...
STIG Check: V-235872
Severity: medium
Rule Title: Docker Enterprise data exchanged between Linux containers on different nodes must be encrypted on the overlay network.
STIG ID: DKER-EE-006240
Rule ID: SV-235872r962034
...
If the network overlay drivers do not show
...
def perform_check(config):
    ...
    # Check Content: Ensure data exchanged between containers are encrypted on different nodes on the overlay network.

via CLI:

Linux: As a Docker EE Admin, follow the steps below using a Universal Control Plane (UCP) c...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - Docker Enterprise data exchanged between Linux containers on different nodes must be encrypted on the overlay network.",
