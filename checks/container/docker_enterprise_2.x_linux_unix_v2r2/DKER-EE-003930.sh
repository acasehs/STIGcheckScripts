...
# STIG Check: V-235842
# Severity: medium
# Rule Title: Docker Trusted Registry (DTR) must be integrated with a trusted certificate authority (CA) in Docker Enterprise.
# STIG ID: DKER-EE-003930
# Rule ID: SV-235842r961596
#
# Description:
...
#     via CLI:
#     
#     Linux: Execute the following command and verify the certificate chai
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-003930 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-003930 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-003930"
    echo "Description: Both the Universal Control Plane (UCP) and DTR components of Docker Enterprise leverage the same authentication and authorization backplane known as eNZi. The eNZi backplane includes its own managed user database, and also allows for LDAP integration in UCP and DTR. To meet the requirements of this control, configure LDAP integration. UCP also includes two certificate authorities for establishing root of trust. One CA is used to sign client bundles and the other is used for TLS communication bet"
