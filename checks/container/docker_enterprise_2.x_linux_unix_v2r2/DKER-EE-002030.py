...
Enabling this option forces containers at runtime to explicitly define their data writing strategy to persist or not persist their data. This also reduces security attack vectors since the container instance's filesystem cannot be tampered with or wr
...
def perform_check(config):
    ...
    # Check Content: Ensure all containers' root filesystem is mounted as read only.

This check should be executed on all nodes in a Docker Enterprise cluster.

via CLI:

Linux: As a Docker EE Admin, execute the followin...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - All Docker Enterprise containers root filesystem must be mounted as read only.",
