...
# STIG Check: V-235805
# Severity: high
# Rule Title: Docker Enterprise hosts network namespace must not be shared.
# STIG ID: DKER-EE-002000
# Rule ID: SV-235805r960963
#
# Description:
...
#     
#     This is potentially dangerous. It allows the container process to open low-numbered ports like any other root process. It also allows the container t
#
# Check Content:
...
#     
#     If the above command returns NetworkMo
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-002000 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-002000 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-002000"
    echo "Description: The networking mode on a container when set to --net=host, skips placing the container inside separate network stack. In essence, this choice tells Docker to not containerize the container's networking. This would network-wise mean that the container lives "outside" in the main Docker host and has full access to its network interfaces.
#     
#     This is potentially dangerous. It allows the container process to open low-numbered ports like any other root process. It also allows the container t"
