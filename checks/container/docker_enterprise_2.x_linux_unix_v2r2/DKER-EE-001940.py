...
STIG Check: V-235800
Severity: medium
Rule Title: SELinux security options must be set on Red Hat or CentOS systems for Docker Enterprise.
STIG ID: DKER-EE-001940
Rule ID: SV-235800r960963
...
docker ps --quiet --all | xargs docker inspect --format '{{ .Name }}: SecurityOpt={{ 
...
def perform_check(config):
    ...
    # Check Content: This check only applies to the use of Docker Engine - Enterprise on either the Red Hat Enterprise Linux or CentOS host operating systems where SELinux is in use and should be executed on all nodes in ...
...
def main():
    parser = argparse.ArgumentParser(
        description=f"STIG Check {STIG_ID} - SELinux security options must be set on Red Hat or CentOS systems for Docker Enterprise.",
