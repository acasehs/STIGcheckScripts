...
# STIG Check: V-235810
# Severity: medium
# Rule Title: Mount propagation mode must not set to shared in Docker Enterprise.
# STIG ID: DKER-EE-002050
# Rule ID: SV-235810r960963
#
# Description:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-002050 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-002050 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-002050"
    echo "Description: Mount propagation mode allows mounting volumes in shared, slave or private mode on a container. Do not use shared mount propagation mode until needed.
#     
#     A shared mount is replicated at all mounts and the changes made at any mount point are propagated to all mounts. Mounting a volume in shared mode does not restrict any other container to mount and make changes to that volume. This unintended volume changes could potentially impact data hosted on the mounted volume. Do not set mount pr"
