...
# STIG Check: V-235787
# Severity: low
# Rule Title: Docker Enterprise must alert the ISSO and SA (at a minimum) in the event of an audit processing failure.
# STIG ID: DKER-EE-001590
# Rule ID: SV-235787r960912
#
# Description:
...
#     
#     Work with the SIEM administrator to determine if an alert is configured when audit data is no longer receive
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-001590 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-001590 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-001590"
