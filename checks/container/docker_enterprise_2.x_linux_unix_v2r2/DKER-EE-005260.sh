...
# STIG Check: V-235860
# Severity: medium
# Rule Title: Docker Enterprise TLS certificate authority (CA) certificate file permissions must be set to 444 or more restrictive.
# STIG ID: DKER-EE-005260
# Rule ID: SV-235860r961863
#
# Description:
...
#     
#     By default, the permissions for TLS CA certificate file might not be 444. The default file permissions 
#
# Check Content:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-005260 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-005260 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-005260"
    echo "Description: Verify that the TLS CA certificate file (the file that is passed along with --TLScacert parameter) has permissions of 444 or more restrictive.
#     
#     The TLS CA certificate file should be protected from any tampering. It is used to authenticate Docker server based on given CA certificate. Hence, it must have permissions of 444 to maintain the integrity of the CA certificate.
#     
#     By default, the permissions for TLS CA certificate file might not be 444. The default file permissions "
