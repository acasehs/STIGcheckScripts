...
# STIG Check: V-235780
# Severity: medium
# Rule Title: LDAP integration in Docker Enterprise must be configured.
# STIG ID: DKER-EE-001100
# Rule ID: SV-235780r960768
#
# Description:
...
#
# Check Content:
#     Verify that LDAP integration is enabled and properly configured in the UCP Admin Settings and verify that the LDAP/AD server is configured per the requirements set forth
...
#     
#     If it is not set to yes and if the LDAP server is not configured then this is a fin
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-001100 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-001100 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-001100"
    echo "Description: Both the Universal Control Plane (UCP) and Docker Trusted Registry (DTR) components of Docker Enterprise leverage the same authentication and authorization backplane known as eNZi. The eNZi backplane provides automated mechanisms for supporting account management functions and allows for LDAP integration in UCP and DTR. While eNZi includes its own managed user database, it is recommended that LDAP integration be configured to more completely satisfy the requirements of this control.
#     
#    "
