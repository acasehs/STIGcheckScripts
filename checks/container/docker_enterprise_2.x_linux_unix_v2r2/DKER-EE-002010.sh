...
# STIG Check: V-235806
# Severity: medium
# Rule Title: Memory usage for all containers must be limited in Docker Enterprise.
# STIG ID: DKER-EE-002010
# Rule ID: SV-235806r960963
#
# Description:
...
#     
#     By default, container can use all of the memory on the host. The user can use memory limit mechanism to prevent a denial of service arising from one container consuming all of the hostâ€™s resources such that other containers on the same host cannot perform the
#
# Check Content:
...
#     
#     If the above command returns 0, it means the memory limits are not in place and this
#
# Exit Codes:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-002010 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-002010 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-002010"
    echo "Description: By default, all containers on a Docker host share the resources equally. By using the resource management capabilities of Docker host, such as memory limit, the amount of memory that a container may consume can be controlled.
#     
#     By default, container can use all of the memory on the host. The user can use memory limit mechanism to prevent a denial of service arising from one container consuming all of the hostâ€™s resources such that other containers on the same host cannot perform the"
