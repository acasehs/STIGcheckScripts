...
# STIG Check: V-235861
# Severity: high
# Rule Title: Docker Enterprise server certificate file ownership must be set to root:root.
# STIG ID: DKER-EE-005270
# Rule ID: SV-235861r961863
#
# Description:
...
#     
#     By default, the ownership and group-ownership for Docker server certificate file is cor
#
# Check Content:
...
main() {
    ...
    #     echo "PASS: Check DKER-EE-005270 - Compliant"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "PASS" "Compliant" "$output"
    #     exit 0
    # else
    #     echo "FAIL: Check DKER-EE-005270 - Finding"
    #     [[ -n "$OUTPUT_JSON" ]] && output_json "FAIL" "Non-compliant" "$output"
    #     exit 1
    # fi

    echo "TODO: Implement check logic for DKER-EE-005270"
    echo "Description: Verify that the Docker server certificate file (the file that is passed along with --TLScert parameter) is owned and group-owned by root.
#     
#     The Docker server certificate file should be protected from any tampering. It is used to authenticate Docker server based on the given server certificate. Hence, it must be owned and group-owned by root to maintain the integrity of the certificate.
#     
#     By default, the ownership and group-ownership for Docker server certificate file is cor"
