...
# Severity: medium
# Rule Title: System privileges granted using the WITH ADMIN OPTION must not be gran...
#
...
QUERY="select grantee, privilege from dba_sys_privs where grantee not in (<list of nonapplicable accounts>) and admin_option = 'YES' and grantee not in (select grantee from dba_role_privs where granted_role = 'DBA')"
