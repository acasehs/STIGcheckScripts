...
#
# Check Content:
#     A default Oracle Database installation provides a set of predefined administrative accounts and nonadministrative accounts. These are accounts that have special privileges required to administer areas of the database, such as the \"CREATE ANY TABLE\" or \"ALTER SESSION\" privilege, or \"EXECUTE\" privileges on packages owned by the SYS schema. The default tablespace for administrative accounts is either \"SYSTEM\" or \"SYSAUX\". Nonadministrative user accounts only have the minimum privileges needed to perform their jobs. Their default tablespace is \"USERS\"._x000D_
_x000D_
To protect these accounts from unauthorized access, the installation process expires and locks most of these accounts, except where noted below. The database administrator is responsible for unlocking and resetting these accounts, as required._x000D_
_x000D_
Non-Administrative Accounts - Expired and locked:_x000D_
APEX_PUBLIC_USER, DIP, FLOWS_040100*, FLOWS_FILES, MDDATA, SPATIAL_WFS_ADMIN_USR, XS$NULL_x000D_
_x000
#
# Exit Codes:
