...
select log_mode from v\$database; select value from v\$parameter where name = \\'log_archive_dest\\';
EXIT;
EOSQL
)
    ...
    echo "Finding Condition: If a value is not returned for LOG_ARCHIVE_DEST and no values are returned for any of the LOG_ARCHIVE_DEST_[1-10] parameters, and no value is returned for DB_RECOVERY_FILE_DEST, this is a finding."
    echo ""
