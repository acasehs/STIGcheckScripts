...
# Severity: medium
# Rule Title: Oracle roles granted using the WITH ADMIN OPTION must not be granted t...
#
...
QUERY="select grantee||': '||granted_role from dba_role_privs where grantee not in (<list of nonapplicable accounts>) and admin_option = 'YES' and grantee not in (select distinct owner from dba_objects) and grantee not in (select grantee from dba_role_privs where granted_role = 'DBA') order by grantee"
