...
# STIG Check: V-270521
# Severity: medium
# Rule Title: Oracle instance names must not contain Oracle version numbers.
# STIG ID: O19C-00-008600
# Rule ID: SV-270521r1112467
#
# Description:
...
#
# Check Content:
#     If using a non-CDB database:  From SQL*Plus:  select instance_name, version from v$instance;  If using a CDB database:  To check the container database (CDB):  From SQL*Plus:  select instance_name, version from v$instance;  To check the pluggable databases (PDBs) within the CDB:  select name from v$...
#
# Exit Codes:
    ...
    cat > "$OUTPUT_JSON" << EOF_JSON
{
  ...
  "stig_id": "$STIG_ID",
  "severity": "$SEVERITY",
  "rule_title": "Oracle instance names must not contain Oracle version numbers.",
  "status": "$STATUS",
